...
if (-not $env:DBA_BROWSER_SUITE) {
    ...
    try {
        foreach ($dbaSuite in @('native','yolo','tree-context','workspace-toolbar','script-selection','grid','table-designer','view-query','query-builder','object-creation','object-designer','grid-edit','project-context','editor-pairing','approvals','approval-review','core')) {
