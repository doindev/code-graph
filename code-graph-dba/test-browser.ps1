...
if (-not $env:DBA_BROWSER_SUITE) {
    ...
    try {
        foreach ($dbaSuite in @('driver-downloads','native','yolo','tree-context','workspace-toolbar','script-selection','grid','grid-values','editable-grid','grid-scroll','grid-search','grid-cell','table-designer','view-query','query-builder','object-creation','object-designer','grid-edit','project-context','catalog','editor-pairing','approvals','approval-review','core')) {
