...
module.exports=async(browser,base,jar,schema='')=>{
 ...
 try{
  ...
  await page.getByRole('tab',{name:'Data',exact:true}).click();for(const name of ['Edit','Add row','Delete'])assert.equal(await page.locator('.table-panel .data-grid-footer').getByRole('button',{name,exact:true}).isDisabled(),true,'Unverified H2 view row writes stay disabled');
