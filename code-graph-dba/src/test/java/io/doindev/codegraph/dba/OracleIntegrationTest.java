...
class OracleIntegrationTest {
    ...
    @Test @Timeout(600) void oracleAllCatalogCategoriesNativeDefinitionsAndGrantCoverage()throws Exception{
        String user="CG_CAT_"+UUID.randomUUID().toString().replace("-","").substring(0,10).toUpperCase(),password="Cg"+UUID.randomUUID().toString().replace("-","");
        try(var profiles=new Profiles(directory,new DbaTest.MemoryVault());var connections=new Connections(profiles);var jobs=new QueryJobs(connections,new DbaConfig(directory,128L<<20,2,100,100,120),x->true)){
            String admin=profiles.put(null,systemDraft()).path("id").asText(),id="";boolean queue=false;long legacy=0;
            try(var c=connections.open(admin);var st=c.createStatement()){
                st.execute("CREATE USER "+user+" IDENTIFIED BY "+OracleDialect.identifier(password)+" QUOTA 40M ON USERS");st.execute("GRANT CREATE SESSION,CREATE TABLE,CREATE VIEW,CREATE MATERIALIZED VIEW,CREATE PROCEDURE,CREATE TYPE,CREATE SEQUENCE,CREATE SYNONYM,CREATE DATABASE LINK,CREATE TRIGGER,CREATE JOB,CREATE RULE,CREATE RULE SET,CREATE EVALUATION CONTEXT TO "+user);st.execute("GRANT EXECUTE ON SYS.DBMS_AQADM TO "+user);
            }
            try{
                id=profiles.put(null,draft(user,password)).path("id").asText();try(var c=connections.open(id);var st=c.createStatement()){
                    st.execute("CREATE TABLE ITEMS(id NUMBER PRIMARY KEY,label VARCHAR2(30))");st.execute("CREATE VIEW ITEM_VIEW AS SELECT id,label FROM ITEMS");st.execute("CREATE MATERIALIZED VIEW ITEM_MV BUILD DEFERRED REFRESH COMPLETE ON DEMAND AS SELECT id FROM ITEMS");st.execute("CREATE INDEX LABEL_IDX ON ITEMS(label)");st.execute("CREATE SEQUENCE COUNTER START WITH 9 CACHE 5");
                    st.execute("CREATE TYPE VALUE_TYPE AS OBJECT(n NUMBER,MEMBER FUNCTION amount RETURN NUMBER)");st.execute("CREATE TYPE BODY VALUE_TYPE AS MEMBER FUNCTION amount RETURN NUMBER IS BEGIN RETURN self.n; END; END;");
                    st.execute("CREATE PACKAGE API AS PROCEDURE RUN_JOB; END;");st.execute("CREATE PACKAGE BODY API AS PROCEDURE RUN_JOB IS BEGIN NULL; END; END;");st.execute("CREATE PROCEDURE WORK AS BEGIN NULL; END;");st.execute("CREATE FUNCTION AMOUNT RETURN NUMBER AS BEGIN RETURN 1; END;");st.execute("CREATE SYNONYM ITEM_ALIAS FOR ITEMS");
                    st.execute("CREATE TRIGGER ITEM_TRIGGER BEFORE INSERT ON ITEMS FOR EACH ROW BEGIN NULL; END;");st.execute("CREATE TRIGGER OWNER_TRIGGER AFTER LOGON ON SCHEMA BEGIN NULL; END;");
                    st.execute("CREATE DATABASE LINK REMOTE_UNUSED CONNECT TO fixture_user IDENTIFIED BY \"fixture-only\" USING 'fixture-not-resolvable'");
                    st.execute("CREATE OR REPLACE AND COMPILE JAVA SOURCE NAMED \"CgCatalogJava\" AS public class CgCatalogJava { public static int amount() { return 1; } }");
                    st.execute("BEGIN SYS.DBMS_AQADM.CREATE_QUEUE_TABLE(queue_table=>'EVENT_TABLE',queue_payload_type=>'SYS.AQ$_JMS_TEXT_MESSAGE'); END;");queue=true;st.execute("BEGIN SYS.DBMS_AQADM.CREATE_QUEUE(queue_name=>'EVENT_QUEUE',queue_table=>'EVENT_TABLE'); END;");
                    st.execute("BEGIN SYS.DBMS_SCHEDULER.CREATE_PROGRAM('WORK_PROGRAM','STORED_PROCEDURE','WORK',0,false);SYS.DBMS_SCHEDULER.CREATE_SCHEDULE('WORK_SCHEDULE',TO_TIMESTAMP_TZ('2099-01-01 +00:00','YYYY-MM-DD TZH:TZM'),'FREQ=DAILY');SYS.DBMS_SCHEDULER.CREATE_JOB(job_name=>'WORK_JOB',program_name=>'WORK_PROGRAM',schedule_name=>'WORK_SCHEDULE',enabled=>false,auto_drop=>false); END;");
                    st.execute("CREATE PROCEDURE WORK_VALUE(value VARCHAR2) AS BEGIN NULL; END;");
                    st.execute("BEGIN SYS.DBMS_SCHEDULER.CREATE_PROGRAM('ARG_PROGRAM','STORED_PROCEDURE','WORK_VALUE',1,false);SYS.DBMS_SCHEDULER.DEFINE_PROGRAM_ARGUMENT(program_name=>'ARG_PROGRAM',argument_position=>1,argument_name=>'VALUE',argument_type=>'VARCHAR2',default_value=>'default fixture');SYS.DBMS_SCHEDULER.CREATE_JOB(job_name=>'ARG_JOB',program_name=>'ARG_PROGRAM',schedule_name=>'WORK_SCHEDULE',enabled=>false,auto_drop=>false);SYS.DBMS_SCHEDULER.SET_JOB_ARGUMENT_VALUE('ARG_JOB',1,'job fixture'); END;");
                    st.execute("BEGIN SYS.DBMS_SCHEDULER.CREATE_CHAIN('WORK_CHAIN');SYS.DBMS_SCHEDULER.DEFINE_CHAIN_STEP('WORK_CHAIN','FIRST','WORK_PROGRAM');SYS.DBMS_SCHEDULER.DEFINE_CHAIN_RULE('WORK_CHAIN','TRUE','START FIRST');SYS.DBMS_SCHEDULER.DEFINE_CHAIN_RULE('WORK_CHAIN','FIRST COMPLETED','END'); END;");
                    try(var call=c.prepareCall("BEGIN SYS.DBMS_JOB.SUBMIT(?, 'BEGIN NULL; END;', TO_DATE('2099-01-01','YYYY-MM-DD'), 'SYSDATE+1'); END;")){call.registerOutParameter(1,Types.NUMERIC);call.execute();legacy=call.getLong(1);}c.commit();
                    st.execute("GRANT SELECT ON ITEMS TO PUBLIC");st.execute("GRANT UPDATE(label) ON ITEMS TO PUBLIC");
                    // An owner-local catalog name must never intercept fixed application metadata SQL.
                    st.execute("CREATE VIEW ALL_OBJECTS AS SELECT CAST(NULL AS VARCHAR2(1)) wrong_column FROM SYS.DUAL WHERE 1=0");
                }
                for(String label:VendorMetadata.categories("oracle")){
                    String kind=VendorMetadata.kind(label);var submitted=jobs.metadataTree("browser",id,Profiles.JSON.createObjectNode().put("kind",kind).put("database","FREEPDB1").put("schema",user));JsonNode page=complete(jobs,submitted);assertFalse(page.path("nodes").isEmpty(),label+": "+page);jobs.remove("browser",submitted.path("id").asText());System.out.println("ORACLE_CATALOG_CATEGORY "+label);
                }
                var program=ObjectDesignerTest.load(jobs,id,MetadataActionsTest.selection(jobs,id,"scheduler_programs",user,"WORK_PROGRAM"));assertTrue(program.path("ddlComplete").asBoolean(),program.toPrettyString());assertTrue(program.path("ddl").asText().toLowerCase().contains("create_program"));
                var chain=ObjectDesignerTest.load(jobs,id,MetadataActionsTest.selection(jobs,id,"scheduler_chains",user,"WORK_CHAIN"));assertFalse(chain.path("ddlComplete").asBoolean(),chain.toPrettyString());assertEquals(2,chain.path("details").path("Chain rules").size());assertEquals("WORK_PROGRAM",chain.path("details").path("Chain steps").path(0).path("program_name").asText());
                try(var c=connections.open(id);var docs=io.doindev.codegraph.store.DocumentStore.memory(64L<<20)){
                    var scope=Profiles.JSON.createObjectNode().put("database","FREEPDB1").put("schema",user);ObjectNode[] observed={null};var phases=new java.util.HashSet<String>();String connectionId=id;
                    docs.replace(writer->{try{var scanner=new CatalogScanner(c,profiles.get(connectionId),scope,writer,()->false,CatalogScanner.Limits.DEFAULT,_->{},jobs::statementTimeoutSeconds);scanner.onProgress((phase,objects,dependencies,bytes)->{if(phases.add(phase))System.out.println("ORACLE_SCAN_PHASE "+phase+" objects="+objects);});observed[0]=scanner.scan();}catch(Exception failure){throw new RuntimeException(failure);}});
                    assertTrue(observed[0].path("inventoryComplete").asBoolean(),observed[0].toPrettyString());assertTrue(observed[0].path("finishedAt").asLong()>=observed[0].path("startedAt").asLong());assertTrue(phases.contains("native_inventory"),phases.toString());
                    var kinds=new java.util.HashSet<String>();boolean[] programDdl={false},columnGrant={false},linkSafe={false},chainCovered={false},argumentsCovered={false};docs.scan("o/",(key,value)->{JsonNode object=ProjectContexts.json(value);String kind=object.path("kind").asText();kinds.add(kind);if(kind.equals("scheduler_program")&&object.path("name").asText().equals("WORK_PROGRAM"))programDdl[0]=object.path("ddl").asText().toLowerCase().contains("create_program");if(kind.equals("column_grant")&&object.path("column").asText().equals("LABEL"))columnGrant[0]=object.path("grantor").asText().equals(user)&&object.path("privilege").asText().equals("UPDATE");if(kind.equals("scheduler_chain")&&object.path("name").asText().equals("WORK_CHAIN"))chainCovered[0]=object.path("definitionCoverage").asText().equals("native_fragment")&&object.path("chainRules").size()==2&&object.path("chainSteps").size()==1;if(kind.equals("scheduler_job")&&object.path("name").asText().equals("ARG_JOB"))argumentsCovered[0]=object.path("schedulerArguments").path(0).path("value").asText().equals("job fixture");if(kind.equals("database_link"))linkSafe[0]=object.path("externalCredentialsRequired").asBoolean()&&!object.toString().contains("fixture-only");});
                    assertTrue(kinds.containsAll(java.util.Set.of("table","view","materialized_view","index","sequence","queue","type","type_body","package","package_body","procedure","function","synonym","trigger","database_link","java_source","job","scheduler_job","scheduler_program","scheduler_schedule","scheduler_chain","grant","column_grant")),kinds.toString());assertTrue(programDdl[0]);assertTrue(columnGrant[0]);assertTrue(linkSafe[0]);assertTrue(chainCovered[0]);assertTrue(argumentsCovered[0]);boolean[] schedulerEdge={false};docs.scan("e/",(key,value)->{JsonNode edge=ProjectContexts.json(value);if(edge.path("name").asText().equals("WORK_CHAIN")&&edge.path("target").asText().equals("WORK_PROGRAM"))schedulerEdge[0]=true;});assertTrue(schedulerEdge[0]);c.rollback();
                    docs.replace(writer->{SQLException denied=assertThrows(SQLException.class,()->new CatalogScanner(c,profiles.get(connectionId),Profiles.JSON.createObjectNode().put("database","FREEPDB1").put("schema","SYSTEM"),writer,()->false).scan());assertTrue(denied.getMessage().contains("SELECT_CATALOG_ROLE"),denied.toString());});
                }
                System.out.println("ORACLE_ALL_CATALOG_CATEGORIES_VERIFIED");
            }finally{
                if(!id.isEmpty()){try(var c=connections.open(id);var st=c.createStatement()){if(legacy>0)st.execute("BEGIN SYS.DBMS_JOB.REMOVE("+legacy+"); END;");if(queue)st.execute("BEGIN SYS.DBMS_AQADM.DROP_QUEUE_TABLE('EVENT_TABLE',force=>TRUE); END;");c.commit();}finally{connections.remove(id);}}
                try(var c=connections.open(admin);var st=c.createStatement()){st.execute("DROP USER "+user+" CASCADE");}
            }
        }
    }
    ...
    @Test @Timeout(300) void oracleIdentitySequenceValuesAdvanceThroughOwningTables()throws Exception{
        String suffix=UUID.randomUUID().toString().replace("-","").substring(0,10).toUpperCase(),source="CG_IS_"+suffix,destination="CG_ID_"+suffix,password="Cg"+UUID.randomUUID().toString().replace("-","");
        try(var profiles=new Profiles(directory,new DbaTest.MemoryVault());var connections=new Connections(profiles);var jobs=new QueryJobs(connections,new DbaConfig(directory,384L<<20,2,100,100,120),x->true);var compare=new DatabaseCompare(profiles,connections,jobs,directory,x->true)){
            String admin=profiles.put(null,systemDraft()).path("id").asText(),from="",to="";
            try(var c=connections.open(admin);var st=c.createStatement()){for(String owner:java.util.List.of(source,destination)){st.execute("CREATE USER "+owner+" IDENTIFIED BY "+OracleDialect.identifier(password)+" QUOTA 10M ON USERS");st.execute("GRANT CREATE SESSION,CREATE TABLE,CREATE SEQUENCE,SELECT_CATALOG_ROLE TO "+owner);}}
            try{
                from=profiles.put(null,draft(source,password)).path("id").asText();to=profiles.put(null,draft(destination,password)).path("id").asText();
                for(String id:java.util.List.of(from,to))try(var c=connections.open(id);var st=c.createStatement()){
                    for(String table:java.util.List.of("CACHED_ID","AHEAD_ID"))st.execute("CREATE TABLE "+table+"(id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY(START WITH 7 INCREMENT BY 3 MINVALUE 1 MAXVALUE 999 CACHE 5 ORDER),label VARCHAR2(30))");
                    st.execute("CREATE TABLE DESC_ID(id NUMBER GENERATED ALWAYS AS IDENTITY(START WITH -7 INCREMENT BY -3 MINVALUE -999 MAXVALUE -1 NOCACHE NOORDER),label VARCHAR2(30))");
                    if(id.equals(from)){st.execute("CREATE TABLE NEW_ID(id NUMBER GENERATED BY DEFAULT AS IDENTITY(START WITH 20 INCREMENT BY 2 MINVALUE 1 MAXVALUE 999 NOCACHE),label VARCHAR2(30))");for(String table:java.util.List.of("CACHED_ID","AHEAD_ID","DESC_ID","NEW_ID")){st.execute("INSERT INTO "+table+"(label) VALUES('first')");st.execute("INSERT INTO "+table+"(label) VALUES('second')");}}
                    else st.execute("ALTER TABLE AHEAD_ID MODIFY(id GENERATED AS IDENTITY(START WITH 601 INCREMENT BY 3 MINVALUE 1 MAXVALUE 999 CACHE 5 ORDER NOCYCLE NOKEEP NOSCALE))");c.commit();
                }
                var before=new java.util.TreeMap<String,java.math.BigInteger>();try(var c=connections.open(from);var st=c.createStatement();var rows=st.executeQuery("SELECT i.table_name,s.last_number FROM SYS.USER_TAB_IDENTITY_COLS i JOIN SYS.USER_SEQUENCES s ON s.sequence_name=i.sequence_name ORDER BY i.table_name")){while(rows.next())before.put(rows.getString(1),rows.getBigDecimal(2).toBigIntegerExact());}
                var input=Profiles.JSON.createObjectNode().put("sourceReceipt",DatabaseCompareTest.receipt(compare,jobs,from,source)).put("destinationReceipt",DatabaseCompareTest.receipt(compare,jobs,to,destination)).put("dataMode","none").put("syncSequences",false);input.putArray("objectTypes").add("sequences");
                String id=compareFinished(jobs,compare.start("human",input)).path("comparisonId").asText();var results=compare.results("human",id,0,100,"","");assertEquals(0,results.path("counts").path("unsupported").asInt(),results.toPrettyString());assertEquals(8,results.path("objects").size(),results.toPrettyString());
                var selection=DatabaseCompareTest.select(compare,id).put("dataMode","none").put("syncSequences",true).put("destructiveSchema",true);var artifact=compareFinished(jobs,compare.generate("human",id,selection));String script=compare.artifacts.preview("human",artifact.path("artifactId").asText()).path("sql").asText();assertFalse(script.contains("ALTER SEQUENCE"),script);assertTrue(SqlScript.extract(script,"oracle",1<<20).stream().noneMatch(unit->OracleCompareSql.canonical(unit.sql()).contains("NEXTVAL")),script);assertTrue(script.contains("GENERATED AS IDENTITY (START WITH"),script);assertTrue(script.indexOf("CREATE TABLE")<script.indexOf("GENERATED AS IDENTITY (START WITH"),script);
                try(var c=connections.open(from);var st=c.createStatement();var rows=st.executeQuery("SELECT i.table_name,s.last_number FROM SYS.USER_TAB_IDENTITY_COLS i JOIN SYS.USER_SEQUENCES s ON s.sequence_name=i.sequence_name")){while(rows.next())assertEquals(before.get(rows.getString(1)),rows.getBigDecimal(2).toBigIntegerExact(),"Comparison/generation must not consume source values");}
                try(var c=connections.open(to);var st=c.createStatement()){
                    ...
                    try(var rows=st.executeQuery("SELECT i.table_name,i.generation_type,s.last_number,s.increment_by,s.cache_size,s.order_flag,s.cycle_flag FROM SYS.USER_TAB_IDENTITY_COLS i JOIN SYS.USER_SEQUENCES s ON s.sequence_name=i.sequence_name")){int count=0;while(rows.next()){count++;String table=rows.getString(1);assertEquals(table.equals("AHEAD_ID")?new java.math.BigInteger("601"):before.get(table),rows.getBigDecimal(3).toBigIntegerExact(),table);assertEquals(table.equals("DESC_ID")?"ALWAYS":"BY DEFAULT",rows.getString(2));assertEquals(table.equals("DESC_ID")?-3:table.equals("NEW_ID")?2:3,rows.getInt(4));assertEquals(table.equals("CACHED_ID")||table.equals("AHEAD_ID")?5:0,rows.getInt(5));assertEquals(table.equals("CACHED_ID")||table.equals("AHEAD_ID")?"Y":"N",rows.getString(6));assertEquals("N",rows.getString(7));}assertEquals(4,count);}
                    st.execute("INSERT INTO CACHED_ID(id,label) VALUES(NULL,'on null retained')");try(var rows=st.executeQuery("SELECT id FROM CACHED_ID WHERE label='on null retained'")){assertTrue(rows.next());assertEquals(before.get("CACHED_ID"),rows.getBigDecimal(1).toBigIntegerExact());}c.commit();
                }
                compare.remove("human",id);id=compareFinished(jobs,compare.start("human",input)).path("comparisonId").asText();results=compare.results("human",id,0,100,"","");assertEquals(8,results.path("counts").path("identical").asInt(),results.toPrettyString());compare.remove("human",id);System.out.println("ORACLE_IDENTITY_SEQUENCE_SYNC_VERIFIED");
