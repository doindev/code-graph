...
class ProjectContextsTest {
    ...
    @Test void everyTemplateHasAProviderPathAndUnknownSqlAlwaysRequiresApproval()throws Exception{
        ...
        try(Connection c=connections.open(connection)){for(var template:DatabaseCatalog.ALL)try(var docs=DocumentStore.memory(16L<<20)){ObjectNode profile=profiles.get(connection).put("templateId",template.id());if(template.id().equals("oracle")){docs.replace(w->{var failure=assertThrows(SQLException.class,()->new CatalogScanner(c,profile,contexts.binding(binding),w,()->false).scan());assertTrue(failure.getMessage().contains("not an Oracle database"));});continue;}ObjectNode[] snapshot={null};docs.replace(w->{try{snapshot[0]=new CatalogScanner(c,profile,contexts.binding(binding),w,()->false).scan();}catch(Exception e){throw new RuntimeException(template.id(),e);}});assertTrue(snapshot[0].path("objects").asInt()>=2,template.id());assertEquals("H2",snapshot[0].path("version").path("product").asText());c.rollback();}}
