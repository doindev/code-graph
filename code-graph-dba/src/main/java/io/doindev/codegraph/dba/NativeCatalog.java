...
final class NativeCatalog {
    ...
    static ObjectNode describe(com.fasterxml.jackson.databind.JsonNode profile,NativeTarget target,boolean approvalsEnabled){
        ...
        if(target.transport()==DatabaseTransport.MONGODB)for(String operation:List.of("guardedDocumentCreation","guardedDocumentReplacement","guardedDocumentDeletion"))operations.putObject(operation)
        ...
        if(target.transport()==DatabaseTransport.REDIS)operations.putObject("transactions")
                ...
                .put("guardedListEndDeletion",true).put("maximumListLength",NativeRedisTransactions.MAX_LIST_LENGTH)
                .put("listEndDeletionRestriction","Exactly one LTRIM key 1 -1 or LTRIM key 0 -2, with one same-key end index/length/complete-value WATCH guard. No raw/pipeline trim, arbitrary range or mixed batch. Last-item deletion removes the key and TTL.")
                .put("restriction","Reviewed transaction object containing supported scalar-reply mutations and optional string/absent WATCH expectations. Optional watch.field checks a hash field on an existing hash; Redis 7.4+ and HPTTL permission required, expiring fields rejected. Optional watch.index/length checks current position/value in lists of at most 10000 items; positions are not stable identities. Optional watch.member checks boolean membership in an existing set (8 KiB member maximum); false never permits recreating a missing set. Optional watch.scoreMember checks a finite decimal-string score, or explicit null for an absent member, inside an existing sorted set; never creates a missing key. No mixed kinds or nonfinite scores. Removing the last member deletes the key and TTL. GEO indexes also use zset, so TYPE does not prove score semantics. One expectation per key. Cluster requires one hash slot; no scripts, read arrays, cross-slot routing, rollback or retries.");
