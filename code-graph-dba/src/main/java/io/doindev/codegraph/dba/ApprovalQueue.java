...
final class ApprovalQueue implements AutoCloseable {
    ...
    private JsonNode request(String principal,String mcpSession,JsonNode input,String trustedCategory,boolean requireReusableRead){
        ...
        ObjectNode matched=reusable.match(principal,mcpSession,value,permissionScope,operation);
        ...
        Request r=new Request(value,principal,bound?contexts.hold(bindingId):()->{});r.mcpSession=mcpSession;r.scope=permissionScope;r.operation=operation;
        ...
        else if(verifiedRead&&!input.has("database")&&!input.has("schema")&&agents.permitsRead(principal,"query",b)){value.put("authorizationReason","Legacy read policy (unchanged scope)");submit(r,"legacy_policy");}
    ...
    private ObjectNode status(Request r,boolean browser){ObjectNode out=r.value.deepCopy();out.remove(List.of("profileRevision","requestHash","reusableScope"));out.put("state",r.state);if(browser)out.put("agentId",r.principal);if(!r.jobId.isEmpty()){out.put("jobId",r.jobId);try{JsonNode job=jobs.status("agent:"+r.principal,r.jobId);if(browser){ObjectNode summary=((ObjectNode)job).deepCopy();summary.remove(List.of("result","exception"));out.set("job",summary);}else out.set("job",job);String state=job.path("state").asText();if(Set.of("complete","failed","cancelled").contains(state)&&!r.state.equals(state)){r.state=state;r.release();out.put("state",state);record(r,state,"");}}catch(Exception e){out.put("jobExpired",true);if(r.state.equals("submitted")){r.state="result_expired";r.release();out.put("state",r.state);record(r,r.state,"");}}}return out;}
