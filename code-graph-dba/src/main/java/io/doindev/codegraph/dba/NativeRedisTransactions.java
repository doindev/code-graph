...
final class NativeRedisTransactions {
    ...
    static NativeCommand.Classification classify(NativeTarget target,JsonNode command){
        ...
        boolean destructive=false,hashDeletion=false,listEdit=false;
        ...
        if(command.has("watch")){
            ...
            for(JsonNode item:watch){
                fields(item,Set.of("key","field","index","length","expected"));
        ...
        try(var session=NativeRedisSession.open(lease,target,job.remainingSeconds())){
            ...
            if(!watch.isMissingNode()&&!watch.isEmpty()){
                ...
                for(int i=0;i<watch.size();i++){
                    ...
                    boolean matches=watch.get(i).has("index")?listMatches(redis,watched[i],watch.get(i),type):watch.get(i).has("field")?hashMatches(redis,watched[i],watch.get(i),type):expected.isNull()?type.equals("none"):type.equals("string")
