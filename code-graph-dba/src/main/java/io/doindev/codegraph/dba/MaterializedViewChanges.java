...
final class MaterializedViewChanges {
    static void capture(QueryJobs.Job job,Connection c,ObjectNode out,String oid)throws Exception{
        ...
        retained.put("currentUser",ObjectCatalog.query(job,c,"SELECT current_user AS name").path(0).path("name").asText());
        retained.put("specialized",ObjectCatalog.truth(ObjectCatalog.query(job,c,"SELECT EXISTS(SELECT 1 FROM pg_attribute WHERE attrelid=?::oid AND (attacl IS NOT NULL OR attoptions IS NOT NULL)) OR EXISTS(SELECT 1 FROM pg_statistic_ext WHERE stxrelid=?::oid) OR EXISTS(SELECT 1 FROM pg_depend WHERE objid=?::oid AND classid='pg_class'::regclass AND refclassid='pg_extension'::regclass) OR EXISTS(SELECT 1 FROM pg_seclabel WHERE objoid=?::oid AND classoid='pg_class'::regclass AND objsubid>0) AS specialized",oid,oid,oid,oid).path(0).path("specialized")));
        retained.set("indexAttributes",ObjectCatalog.query(job,c,"SELECT ci.relname AS name,COALESCE(obj_description(ci.oid,'pg_class'),'') AS comment,i.indisclustered AS clustered FROM pg_index i JOIN pg_class ci ON ci.oid=i.indexrelid WHERE i.indrelid=?::oid",oid));
        retained.set("columns",ObjectCatalog.query(job,c,"SELECT attname AS name,attstattarget::text AS statistics,attstorage::text AS storage,attcompression::text AS compression,COALESCE(col_description(attrelid,attnum),'') AS comment FROM pg_attribute WHERE attrelid=?::oid AND attnum>0 AND NOT attisdropped ORDER BY attnum",oid));
        retained.set("indexes",out.path("details").path("Indexes"));
        retained.set("grants",ObjectCatalog.query(job,c,"SELECT CASE WHEN a.grantee=0 THEN 'PUBLIC' ELSE pg_get_userbyid(a.grantee) END AS grantee,pg_get_userbyid(a.grantor) AS grantor,a.privilege_type,a.is_grantable FROM pg_class c CROSS JOIN LATERAL aclexplode(COALESCE(c.relacl,acldefault('r',c.relowner))) a WHERE c.oid=?::oid",oid));
        retained.set("defaultGrantees",ObjectCatalog.query(job,c,"SELECT DISTINCT CASE WHEN a.grantee=0 THEN 'PUBLIC' ELSE pg_get_userbyid(a.grantee) END AS grantee FROM pg_default_acl d CROSS JOIN LATERAL aclexplode(d.defaclacl) a WHERE d.defaclrole=(SELECT oid FROM pg_roles WHERE rolname=current_user) AND d.defaclobjtype='r' AND (d.defaclnamespace=0 OR d.defaclnamespace=(SELECT oid FROM pg_namespace WHERE nspname=?))",str(f,"schema")));
        retained.set("securityLabels",ObjectCatalog.query(job,c,"SELECT provider,label FROM pg_seclabel WHERE classoid='pg_class'::regclass AND objoid=?::oid AND objsubid=0",oid));
        retained.set("relation",ObjectCatalog.query(job,c,"SELECT am.amname AS method,c.relispopulated,c.reloptions::text AS options FROM pg_class c LEFT JOIN pg_am am ON am.oid=c.relam WHERE c.oid=?::oid",oid).path(0));
