...
final class ObjectCatalog {
    ...
    static void postgres(QueryJobs.Job job,Connection c,ObjectNode out,JsonNode node)throws Exception{
        ...
        if(!catalog.isEmpty()&&!oid.isBlank()){
            JsonNode raw=query(job,c,"SELECT to_jsonb(t)::text AS properties FROM pg_catalog."+catalog+" t WHERE oid=?::oid",oid).path(0);
            ...
            if(!ownerKey.isBlank())f.put("owner",query(job,c,"SELECT pg_get_userbyid(?::oid) AS name",props.path(ownerKey).asText()).path(0).path("name").asText());
            if(!Set.of("roles","databases","tablespaces").contains(k))f.put("comment",query(job,c,"SELECT COALESCE(obj_description(?::oid,?),'') AS comment",oid,catalog).path(0).path("comment").asText());
            else f.put("comment",query(job,c,"SELECT COALESCE(shobj_description(?::oid,?),'') AS comment",oid,catalog.equals("pg_roles")?"pg_authid":catalog).path(0).path("comment").asText());
        ...
        switch(k){
            case "sequences"->{
                JsonNode row=query(job,c,"SELECT format_type(seqtypid,NULL) AS type,seqstart::text AS start,seqincrement::text AS increment,seqmin::text AS minimum,seqmax::text AS maximum,seqcache::text AS cache,seqcycle AS cycle FROM pg_sequence WHERE seqrelid=?::oid",oid).path(0);
                for(String key:List.of("type","start","increment","minimum","maximum","cache","cycle"))f.set(key,row.path(key));
                f.put("cycle",truth(row.path("cycle")));f.put("ownedBy",query(job,c,"SELECT quote_ident(n.nspname)||'.'||quote_ident(t.relname)||'.'||quote_ident(a.attname) AS name FROM pg_depend d JOIN pg_class t ON t.oid=d.refobjid JOIN pg_namespace n ON n.oid=t.relnamespace JOIN pg_attribute a ON a.attrelid=t.oid AND a.attnum=d.refobjsubid WHERE d.classid='pg_class'::regclass AND d.objid=?::oid AND d.deptype IN ('a','i')",oid).path(0).path("name").asText(""));
                out.put("ddl","CREATE SEQUENCE "+t+ObjectForms.sequenceOptions(f,true)+";").put("ddlComplete",true);
            }
            case "views","materialized_views"->{
                f.put("query",query(job,c,"SELECT pg_get_viewdef(?::oid,true) AS definition",oid).path(0).path("definition").asText().replaceFirst(";\\s*$",""));
                JsonNode relation=query(job,c,"SELECT COALESCE(s.spcname,'') AS tablespace,c.relispopulated FROM pg_class c LEFT JOIN pg_tablespace s ON s.oid=c.reltablespace WHERE c.oid=?::oid",oid).path(0);
            ...
            case "indexes"->{
                JsonNode row=query(job,c,"SELECT pg_get_indexdef(i.indexrelid) AS ddl,quote_ident(n.nspname)||'.'||quote_ident(t.relname) AS table_name,a.amname AS method,i.indisunique AS unique,i.indisvalid AS valid,COALESCE(pg_get_expr(i.indpred,i.indrelid),'') AS predicate,COALESCE(s.spcname,'') AS tablespace FROM pg_index i JOIN pg_class x ON x.oid=i.indexrelid JOIN pg_class t ON t.oid=i.indrelid JOIN pg_namespace n ON n.oid=t.relnamespace JOIN pg_am a ON a.oid=x.relam LEFT JOIN pg_tablespace s ON s.oid=x.reltablespace WHERE i.indexrelid=?::oid",oid).path(0);
                values(f,row,"table","table_name","method","method","predicate","predicate","tablespace","tablespace");f.put("unique",truth(row.path("unique")));
                f.put("columns",query(job,c,"SELECT string_agg(pg_get_indexdef(?::oid,i,true),', ' ORDER BY i) AS definition FROM generate_series(1,(SELECT indnkeyatts FROM pg_index WHERE indexrelid=?::oid)) i",oid,oid).path(0).path("definition").asText());
                out.put("ddl",str(row,"ddl")+";").put("ddlComplete",true);
            }
            case "types","domains"->{
                JsonNode props=out.path("details").path("Advanced");String typtype=str(props,"typtype");
                if(typtype.equals("e")){f.put("labels",String.join("\n",query(job,c,"SELECT enumlabel AS name FROM pg_enum WHERE enumtypid=?::oid ORDER BY enumsortorder",oid).findValuesAsText("name")));out.put("ddl","CREATE TYPE "+t+" AS ENUM ("+String.join(", ",stringsLines(str(f,"labels")).stream().map(TableDesigner::literal).toList())+");").put("ddlComplete",true);}
                if(typtype.equals("c"))detail(out,"Attributes",query(job,c,"SELECT attname,format_type(atttypid,atttypmod) AS datatype FROM pg_attribute WHERE attrelid=?::oid AND attnum>0 AND NOT attisdropped ORDER BY attnum",str(props,"typrelid")));
                if(typtype.equals("d")){out.put("domain",true);f.put("type",query(job,c,"SELECT format_type(typbasetype,typtypmod) AS type FROM pg_type WHERE oid=?::oid",oid).path(0).path("type").asText());f.put("default",str(props,"typdefault")).put("notNull",props.path("typnotnull").asBoolean());detail(out,"Constraints",query(job,c,"SELECT conname,pg_get_constraintdef(oid) AS definition FROM pg_constraint WHERE contypid=?::oid",oid));out.put("ddl","CREATE DOMAIN "+t+" AS "+str(f,"type")+(str(f,"default").isBlank()?"":" DEFAULT "+str(f,"default"))+(f.path("notNull").asBoolean()?" NOT NULL":"")+";");}
            }
            ...
            case "triggers"->{JsonNode row=query(job,c,"SELECT pg_get_triggerdef(oid,true) AS ddl,tgenabled::text AS enabled FROM pg_trigger WHERE oid=?::oid",oid).path(0);out.put("ddl",str(row,"ddl")+";").put("ddlComplete",true);f.put("enabled",str(row,"enabled"));}
            case "constraints"->{String definition=query(job,c,"SELECT pg_get_constraintdef(?::oid,true) AS ddl",oid).path(0).path("ddl").asText();f.put("definition",definition);out.put("ddl","ALTER TABLE "+ObjectForms.qualified("postgresql",str(parent,"schema"),str(parent,"table"))+" ADD CONSTRAINT "+ObjectForms.q("postgresql",str(f,"name"))+" "+definition+";").put("ddlComplete",true);}
            case "rules"->out.put("ddl",query(job,c,"SELECT pg_get_ruledef(?::oid,true) AS ddl",oid).path(0).path("ddl").asText()).put("ddlComplete",true);
            case "policies"->{JsonNode row=query(job,c,"SELECT pg_get_expr(polqual,polrelid) AS using,pg_get_expr(polwithcheck,polrelid) AS check FROM pg_policy WHERE oid=?::oid",oid).path(0);values(f,row,"using","using","check","check");}
            default->{}
        }
    ...
    static void embedded(QueryJobs.Job job,Connection c,ObjectNode out,JsonNode node)throws Exception{
        ...
        optional(c,out,"Native properties",()->{
            ArrayNode rows=k.equals("schemas")?query(job,c,"SELECT * FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME=?",name):query(job,c,"SELECT * FROM INFORMATION_SCHEMA."+table+" WHERE "+prefix+"_SCHEMA=? AND "+prefix+"_NAME=?",s,name);
    ...
    static void nativeVendor(QueryJobs.Job job,Connection c,ObjectNode out,JsonNode node)throws Exception{
        ...
        optional(c,out,"Native definition",()->{
            if(e.equals("sqlite")){
                ...
                var r=query(job,c,"SHOW CREATE "+type+" "+t);detail(out,"Advanced",r);
                ...
                var r=query(job,c,"SELECT GET_DDL(?,?) AS ddl",label(k).toUpperCase(Locale.ROOT).replace(' ','_'),t);out.put("ddl",r.path(0).path("ddl").asText()).put("ddlComplete",true);
                ...
                var r=query(job,c,"SELECT * FROM "+function+"() WHERE schema_name=? AND "+col+"=?",s,name);detail(out,"Advanced",r);out.put("ddl",r.path(0).path("sql").asText(""));
                ...
                if(!table.isBlank()){var r=query(job,c,"SELECT * FROM SYSCAT."+table+" WHERE "+prefix+"SCHEMA=? AND "+prefix+"NAME=?",s,name);detail(out,"Advanced",r);out.put("ddl",r.path(0).path("text").asText(""));}
            }
        });
    }
