...
final class CatalogQueries {
    ...
    JsonNode read(CatalogCache.Scope scope,String principal,String targetKey,String authorizationIdentity,
        ...
        return read(scope,principal,targetKey,authorizationIdentity,operation,args,references,row->true);
    }
    JsonNode read(CatalogCache.Scope scope,String principal,String targetKey,String authorizationIdentity,
                  String operation,JsonNode args,Function<JsonNode,JsonNode> references,java.util.function.Predicate<JsonNode> permitted){
        ...
        try{return scope.read(published->{
            ...
            if(operation.equals("dba_search_objects")){
                ...
                published.store.scan("i/",(key,value)->{JsonNode row=json(value);if(!permitted.test(row))return;boolean matches=(kind.isBlank()||kind.equals(row.path("kind").asText()))&&(row.path("name").asText().toLowerCase(Locale.ROOT).contains(query)||row.path("schema").asText().toLowerCase(Locale.ROOT).contains(query));if(!matches&&args.path("searchDefinitions").asBoolean()&&query.length()>=3&&(kind.isBlank()||kind.equals(row.path("kind").asText())))matches=json(published.store.get("o/"+row.path("id").asText())).path("ddl").asText().toLowerCase(Locale.ROOT).contains(query);if(matches){if(matched[0]++>=offset)page.add(row);}});
            ...
            String objectId=Profiles.text(args,"objectId",64);JsonNode object=json(published.store.get("o/"+objectId));if(object==null||!permitted.test(object))throw new IllegalArgumentException("Unknown or unapproved object in this snapshot");
            ...
            if(operation.equals("dba_get_database_dependencies")){ArrayNode edges=out.putArray("dependencies");int offset=number(args,"offset",0,0,50000),limit=number(args,"limit",100,1,100);int[] total={0};Page page=new Page(edges,limit);published.store.scan("e/",(k,v)->{JsonNode e=json(v);if(!permitted.test(Profiles.JSON.createObjectNode().put("schema",e.path("schema").asText()).put("name",e.path("name").asText()))||!permitted.test(Profiles.JSON.createObjectNode().put("schema",e.path("targetSchema").asText()).put("name",e.path("target").asText())))return;if(e.path("schema").equals(object.path("schema"))&&e.path("name").equals(object.path("name"))||e.path("targetSchema").equals(object.path("schema"))&&e.path("target").equals(object.path("name"))){if(total[0]++>=offset)page.add(e);}});out.put("truncated",total[0]>offset+edges.size()).put("nextOffset",offset+edges.size());page.metadata(out);return out;}
