...
final class OracleAdminPlans {
    ...
    static ObjectNode prepare(QueryJobs.Job job,Connection c,ObjectNode request)throws Exception{
        ...
        if(id.equals("create_user")){absent(query(job,c,"SELECT USER_ID FROM SYS.DBA_USERS WHERE USERNAME=?",name),"User");identifier(name);String suffix="";if(!request.path("tablespace").asText().isEmpty())suffix+=" DEFAULT TABLESPACE "+identifier(request.path("tablespace").asText());if(!request.path("temporaryTablespace").asText().isEmpty())suffix+=" TEMPORARY TABLESPACE "+identifier(request.path("temporaryTablespace").asText());if(!request.path("profile").asText().isEmpty())suffix+=" PROFILE "+identifier(request.path("profile").asText());secret(commands,"CREATE USER "+identifier(name)+" IDENTIFIED BY ",suffix);}
