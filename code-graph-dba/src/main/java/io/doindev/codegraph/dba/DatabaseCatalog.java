...
final class DatabaseCatalog {
    ...
    static ArrayNode properties(String id){
        ...
        String names=switch(id){
            ...
            case "oracle"->"oracle.net.ssl_server_dn_match,oracle.net.ssl_server_cert_dn,javax.net.ssl.trustStore,javax.net.ssl.trustStorePassword,javax.net.ssl.keyStore,javax.net.ssl.keyStorePassword,oracle.net.CONNECT_TIMEOUT,oracle.jdbc.ReadTimeout,oracle.net.wallet_location,defaultRowPrefetch";
            ...
            default->"";};
        for(String name:names.split(","))if(!name.isEmpty()){boolean secret=sensitive(name);var property=out.addObject().put("name",name).put("description",description(name)).put("category",category(name)).put("secret",secret).put("source","catalog").put("type","string");if(Set.of("sslConnection","tcpKeepAlive","useProxy","trustServerCertificate","integratedSecurity","allowPublicKeyRetrieval","CLIENT_SESSION_KEEP_ALIVE","duckdb.read_only").contains(name)){property.put("type","boolean");property.putArray("choices").add("true").add("false");}if(id.equals("postgresql")&&name.equals("sslmode"))property.putArray("choices").add("disable").add("allow").add("prefer").add("require").add("verify-ca").add("verify-full");}
        return out;
    }
