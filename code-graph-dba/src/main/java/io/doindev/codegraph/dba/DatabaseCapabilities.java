...
final class DatabaseCapabilities {
    ...
    static ObjectNode describe(JsonNode profile,JsonNode scope,JsonNode snapshot,boolean approvalsEnabled) {
        ...
        boolean verified=observed&&Set.of("postgresql","mysql","mariadb","h2","sqlserver").contains(engine);
        ...
        operations.putObject("migrationPreparation").put("available",verified).put("requiresPermission",true).put("reason",verified?"Verified engine adapter; retained reviewed artifact only":"Live verified PostgreSQL, MySQL, MariaDB, H2 or SQL Server identity required");
