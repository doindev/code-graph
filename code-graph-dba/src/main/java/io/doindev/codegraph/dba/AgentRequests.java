...
final class AgentRequests implements AutoCloseable {
    ...
    private void reap(){if(sessions!=null)ledger.reap(sessions);for(Request r:requests.values())if(!Set.of("awaiting_approval","submitted").contains(r.state))releaseAdmission(r);long now=clock.getAsLong();var it=requests.values().iterator();while(it.hasNext()){Request r=it.next();if(r.state.equals("awaiting_approval")&&(now>=r.value.path("expiresAt").asLong()||!agents.alive("agent:"+r.principal))){r.state="expired";pendingOwners.remove(r.value.path("id").asText());r.clear();record(r,"expired","");}if(now-r.value.path("createdAt").asLong()>3_600_000&&!r.state.equals("submitted")){r.clear();releaseAdmission(r);it.remove();retainedCount=requests.size();}}}
    ...
    synchronized void sessionEnded(String session){for(Request r:requests.values())if(Objects.equals(session,r.mcpSession)){if(r.state.equals("submitted"))cancel(r.principal,r.value.path("id").asText());else if(r.state.equals("awaiting_approval")){r.state="expired";r.clear();releaseAdmission(r);pendingOwners.remove(r.value.path("id").asText());}}}
