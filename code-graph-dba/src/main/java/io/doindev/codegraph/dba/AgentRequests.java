...
final class AgentRequests implements AutoCloseable {
    ...
    private ObjectNode status(Request r,boolean browser){ObjectNode out=r.value.deepCopy().put("state",r.state);if(browser)out.put("agentId",r.principal);if(r.result!=null)out.set("result",r.result.deepCopy());if(!r.jobId.isEmpty()){out.put("jobId",r.jobId);try{JsonNode job=jobs.status(jobOwner(r),r.jobId);ObjectNode safe=job.deepCopy();if(safe.path("result").has("receipt"))((ObjectNode)safe.path("result")).remove("receipt");out.set("job",safe);String state=job.path("state").asText();if(Set.of("complete","failed","cancelled").contains(state)&&r.state.equals("submitted")){r.state=state;out.put("state",state);if(agents.authorization.automatic||!typeNeedsReceipt(r)||!state.equals("complete"))r.clear();releaseAdmission(r);record(r,state,"");}}catch(Exception e){out.put("jobExpired",true);}}return out;}
