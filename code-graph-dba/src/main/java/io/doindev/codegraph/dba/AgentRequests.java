...
final class AgentRequests implements AutoCloseable {
    ...
    private void sessionValid(Request r){agents.authorization.requireSession(r.principal,r.mcpSession);if(r.mcpSession!=null&&sessions!=null&&!sessions.alive(r.mcpSession,r.principal))throw new SecurityException("Requesting MCP session expired");}
    synchronized void onPending(Consumer<String> listener){onPending=listener;}
    private record Owner(String principal,String session,long expires){}
    ...
    boolean alive(String owner){if(!owner.startsWith("approval:"))return false;Owner r=pendingOwners.get(owner.substring(9));return r!=null&&clock.getAsLong()<r.expires&&agents.alive("agent:"+r.principal)&&(r.session==null||sessions!=null&&sessions.alive(r.session,r.principal));}
    static final Set<String> TYPES=Set.of("connection_create","connection_update","connection_delete","connection_test","connection_details","binding_create","binding_update","binding_delete","native_command");
    private NativeOperations nativeOperations;
    void nativeOperations(NativeOperations value){nativeOperations=value;}
    ...
    static final class Request{
        final ObjectNode value,payload;final String principal,hash;String state="awaiting_approval",jobId="";JsonNode result;String mcpSession,testReceipt="";boolean admitted;volatile boolean cancellationRequested;
        ...
        void clear(){scrub(payload);testReceipt="";}
    }
    ...
    synchronized JsonNode request(String principal,String type,JsonNode input){return request(principal,null,type,input);}
    synchronized JsonNode request(String principal,String mcpSession,String type,JsonNode input){
        ...
        ObjectNode payload=input.deepCopy();String hash=CatalogScanner.hash(type+"\n"+payload.toString());
        String duplicate=ledger.lookup(principal,mcpSession,requestId,hash);if(duplicate!=null){Request previous=requests.get(duplicate);if(previous==null)throw new IllegalArgumentException("Request result expired; operation will not be replayed (request "+duplicate+")");return status(previous,false);}
        if(agents.authorization.automatic&&Set.of("connection_create","connection_update").contains(type)){
            for(String field:List.of("testBeforeSave","saveUntested","confirmDriverEffects"))if(input.has(field)&&!input.path(field).isBoolean())throw new IllegalArgumentException(field+" must be a boolean");
            if(input.path("testBeforeSave").asBoolean()==input.path("saveUntested").asBoolean())throw new IllegalArgumentException("Specify exactly one of testBeforeSave: true or saveUntested: true");
            for(String field:List.of("reviewerReceipt","reviewerSession","reviewerSaveUntested","receipt"))if(input.has(field)||input.path("profile").has(field))throw new IllegalArgumentException("Browser receipts cannot authorize an agent proposal");
            if(input.has("driverInstall")&&input.path("driverInstall").path("version").asText("latest").equals("latest"))throw new IllegalArgumentException("Explicit driver installation requires a pinned version, not latest");
        }
        ObjectNode value=Profiles.JSON.createObjectNode().put("id",UUID.randomUUID().toString()).put("type",type).put("requestId",requestId).put("purpose",purpose).put("createdAt",clock.getAsLong()).put("expiresAt",clock.getAsLong()+approvalTimeout.getAsInt()*1000L).put("mutation",mutation(type));
        if(type.equals("native_command")&&payload.has("bindingId"))contexts.authorized(principal,Profiles.text(payload,"bindingId",36),true);
        prepare(type,payload,value);
        if(type.equals("native_command"))value.set("approvalChoices",ApprovalQueue.choices(new ReusableOperation.Result(value.path("classification").path("category").asText(),false,false,"Native reusable permissions are not yet verified; exact one-time review is required"),false));
        else if(mutation(type))value.set("approvalChoices",ApprovalQueue.choices(new ReusableOperation.Result("administration",false,false,"Administrative changes require one-time detailed approval"),false));
        Request r=new Request(value,payload,principal,hash);r.mcpSession=mcpSession;if(!capacity.tryAcquire()){r.clear();throw new IllegalArgumentException("Approval queue is full");}r.admitted=true;
        try{ledger.reserve(principal,mcpSession,requestId,hash,value.path("id").asText());record(r,"requested","");}catch(RuntimeException e){releaseAdmission(r);r.clear();throw e;}
        requests.put(value.path("id").asText(),r);trimHistory();pendingOwners.put(value.path("id").asText(),new Owner(principal,mcpSession,value.path("expiresAt").asLong()));
        if(agents.authorization.automatic){AgentAuthorization.approved(value);startAutomatic(r);}else onPending.accept(value.path("id").asText());return status(r,false);
    }
    private void prepare(String type,ObjectNode payload,ObjectNode value){
        if(type.equals("native_command")){
            if(nativeOperations==null)throw new IllegalArgumentException("Native workflows are not available");
            value.setAll(nativeOperations.prepare(payload));
            return;
        }
        if(type.startsWith("binding_")){
            ObjectNode proposed=payload.path("binding").isObject()?(ObjectNode)payload.path("binding"):payload;
            if(type.equals("binding_create")){value.set("after",bindingSummary(proposed));ObjectNode profile=profiles.get(Profiles.text(proposed,"connectionId",36));value.put("connectionId",profile.path("id").asText()).put("connectionRevision",ProjectContexts.profileRevision(profile));}
        ...
        if(type.equals("connection_create")||type.equals("connection_update")){
            ...
            if(payload.path("driverInstall").isObject()){ObjectNode install=driverInstall(payload.path("driverInstall"),supplied.path("templateId").asText("custom"));value.set("driverInstall",install.deepCopy());value.put("requiresDriverInstall",true);value.set("after",preliminaryProfile(supplied,install));}
        ...
        else if(type.equals("connection_details")||type.equals("connection_test")){value.put("targetRevision",ProjectContexts.profileRevision(profiles.get(target.path("connectionId").asText())));value.put("eligiblePersistentRead",true).put("readCapability",type.equals("connection_details")?"connection_details":"connection_test");if(target.has("bindingId")){ObjectNode b=contexts.binding(target.path("bindingId").asText());value.put("bindingRevision",ProjectContexts.profileRevision(b));value.put("projectId",b.path("projectId").asText()).put("environment",b.path("environment").asText()).put("role",b.path("role").asText());}}
    }
    private ObjectNode resolveTarget(JsonNode input,boolean create){
        ...
        if(input.hasNonNull("bindingId")){for(String key:List.of("connectionId","connectionName","database","schema"))if(input.has(key))throw new IllegalArgumentException("A binding fixes the complete target; no overrides are allowed");ObjectNode b=contexts.binding(Profiles.text(input,"bindingId",36));out.put("bindingId",b.path("id").asText()).put("connectionId",b.path("connectionId").asText()).put("connectionName",profiles.get(b.path("connectionId").asText()).path("name").asText());return out;}
        String id=Profiles.text(input,"connectionId",36),name=Profiles.text(input,"connectionName",120);ObjectNode p=profiles.get(id);if(!(agents.authorization.automatic?p.path("name").asText().equals(name):Profiles.nameKey(p.path("name").asText()).equals(Profiles.nameKey(name))))throw new SecurityException("Connection name does not match its stable ID");return out.put("connectionId",id).put("connectionName",p.path("name").asText());
    }
    synchronized JsonNode maybeRead(String principal,String type,JsonNode input){return maybeRead(principal,null,type,input);}
    synchronized JsonNode maybeRead(String principal,String mcpSession,String type,JsonNode input){
        if(agents.authorization.automatic)return request(principal,mcpSession,type,input);
        ...
        return request(principal,mcpSession,type,input);
    }
    private JsonNode executeRead(String principal,String type,ObjectNode target){
        ...
        if(DatabaseTransport.of(profiles.get(connection))!=DatabaseTransport.JDBC)return setup.operation("agent:"+principal,"draft-test",Profiles.JSON.createObjectNode().put("connectionId",connection));
    ...
    private JsonNode apply(String session,Request r,JsonNode options)throws Exception{
        ...
        if(type.equals("native_command")){
            ObjectNode job=nativeOperations.submit("agent:"+r.principal,r.value,r.payload,()->validate(r));
            r.jobId=job.path("id").asText();return job;
        }
        ...
        String existing=type.equals("connection_update")?r.value.path("target").path("connectionId").asText():null;ObjectNode draft=((ObjectNode)r.payload.path("profile")).deepCopy();String receipt=r.testReceipt.isEmpty()?successfulReceipt(r):r.testReceipt,owner="approval:"+r.value.path("id").asText();if(receipt.isEmpty()&&r.payload.hasNonNull("reviewerReceipt")&&session.equals(r.payload.path("reviewerSession").asText())){receipt=r.payload.path("reviewerReceipt").asText();owner=session;}if(!receipt.isEmpty())draft.put("receipt",receipt);else if(options.path("saveUntested").asBoolean()||r.payload.path("reviewerSaveUntested").asBoolean())draft.put("saveUntested",true);else throw new IllegalArgumentException("Test this unchanged proposal successfully or explicitly choose Save untested");
        ObjectNode saved=setup.save(owner,existing,draft);if(existing!=null)connections.remove(existing);if(existing==null)profiles.markAgentCreated(saved.path("id").asText(),r.principal,r.value.path("requestId").asText());
        ObjectNode result=Profiles.agentProfile(profiles.get(saved.path("id").asText())).put("creatorReceivesAccess",false);
        if(agents.authorization.automatic)r.result=result;
        if(type.equals("connection_create")&&r.payload.path("binding").isObject()){ObjectNode binding=(ObjectNode)r.payload.path("binding").deepCopy();binding.put("connectionId",saved.path("id").asText());try{if(r.cancellationRequested)throw new java.util.concurrent.CancellationException();sessionValid(r);if(agents.authorization.automatic)agents.authorization.audit(r.principal,r.mcpSession,"Creating optional binding",binding);result.set("binding",contexts.save(binding));}catch(Exception failure){result.put("partialSuccess",true).put("warning","Connection saved, but binding was not created. Inspect the saved connection before submitting a separate binding request.");}}
        if(!agents.authorization.automatic&&!r.jobId.isEmpty()){try{jobs.remove("approval:"+r.value.path("id").asText(),r.jobId);}catch(IllegalArgumentException ignored){}r.jobId="";}
        return result;
    }
    ...
    private void validate(Request r){
        if(r.cancellationRequested)throw new java.util.concurrent.CancellationException();
        sessionValid(r);
        if(r.value.has("connectionRevision")&&!ProjectContexts.profileRevision(profiles.get(r.value.path("connectionId").asText())).equals(r.value.path("connectionRevision").asText()))throw new IllegalArgumentException("Connection changed after request; submit a new request");
        if(r.value.has("bindingRevision")&&!ProjectContexts.profileRevision(contexts.binding(r.value.path("target").path("bindingId").asText())).equals(r.value.path("bindingRevision").asText()))throw new IllegalArgumentException("Binding changed after request; submit a new request");
        if(!agents.alive("agent:"+r.principal))throw new SecurityException("Agent revoked");String type=r.value.path("type").asText();
        if(type.equals("native_command")){if(r.payload.has("bindingId"))contexts.authorized(r.principal,r.payload.path("bindingId").asText(),true);nativeOperations.validate(r.value,r.payload);}
        if(r.value.path("requiresDriverInstall").asBoolean()&&!agents.authorization.automatic)throw new IllegalArgumentException("Review the proposal and explicitly install or select the requested JDBC driver before approval");
    ...
    synchronized JsonNode cancel(String principal,String id){Request r=require(principal,id);r.cancellationRequested=true;if(r.state.equals("awaiting_approval")){r.state="cancelled";pendingOwners.remove(id);r.clear();record(r,"cancelled","");}else if(r.state.equals("submitted")&&!r.jobId.isEmpty()){try{jobs.cancel(jobs.require(jobOwner(r),r.jobId));}catch(Exception ignored){}record(r,"cancellation_requested","");}return status(r,false);}
    private String jobOwner(Request r){if(agents.authorization.automatic)return "agent:"+r.principal;return Set.of("connection_test","native_command").contains(r.value.path("type").asText())?"agent:"+r.principal:"approval:"+r.value.path("id").asText();}
    ...
    private ObjectNode status(Request r,boolean browser){ObjectNode out=r.value.deepCopy().put("approvalId",r.value.path("id").asText()).put("state",r.state);if(browser)out.put("agentId",r.principal);if(r.result!=null)out.set("result",r.result.deepCopy());if(!r.jobId.isEmpty()){out.put("jobId",r.jobId);try{JsonNode job=jobs.status(jobOwner(r),r.jobId);ObjectNode safe=job.deepCopy();if(safe.path("result").has("receipt"))((ObjectNode)safe.path("result")).remove("receipt");out.set("job",safe);String state=job.path("state").asText();if(Set.of("complete","failed","cancelled").contains(state)&&r.state.equals("submitted")){r.state=state;out.put("state",state);if(agents.authorization.automatic||!typeNeedsReceipt(r)||!state.equals("complete"))r.clear();releaseAdmission(r);record(r,state,"");}}catch(Exception e){out.put("jobExpired",true);}}return out;}
    ...
    private void reap(){if(sessions!=null)ledger.reap(sessions);for(Request r:requests.values())if(!Set.of("awaiting_approval","submitted").contains(r.state))releaseAdmission(r);long now=clock.getAsLong();var it=requests.values().iterator();while(it.hasNext()){Request r=it.next();if(r.state.equals("awaiting_approval")&&(now>=r.value.path("expiresAt").asLong()||!agents.alive("agent:"+r.principal))){r.state="expired";r.value.put("failureCode",clock.getAsLong()>=r.value.path("expiresAt").asLong()?"approval_timeout":"access_revoked");pendingOwners.remove(r.value.path("id").asText());r.clear();record(r,"expired","");}if(now-r.value.path("expiresAt").asLong()>3_600_000&&!r.state.equals("submitted")){r.clear();releaseAdmission(r);it.remove();retainedCount=requests.size();}}}
    synchronized void tick(){reap();for(Request r:requests.values())if(r.state.equals("submitted")){if(r.mcpSession!=null&&sessions!=null&&!sessions.alive(r.mcpSession,r.principal))cancel(r.principal,r.value.path("id").asText());status(r,false);}trimHistory();}
    private void releaseAdmission(Request r){if(r.admitted){r.admitted=false;capacity.release();}}
    private void trimHistory(){var it=requests.values().iterator();while(requests.size()>64&&it.hasNext()){Request r=it.next();if(!Set.of("awaiting_approval","submitted").contains(r.state)){r.clear();releaseAdmission(r);it.remove();}}retainedCount=requests.size();}
    synchronized void sessionEnded(String session){for(Request r:requests.values())if(Objects.equals(session,r.mcpSession)){if(r.state.equals("submitted"))cancel(r.principal,r.value.path("id").asText());else if(r.state.equals("awaiting_approval")){r.state="expired";r.value.put("failureCode",clock.getAsLong()>=r.value.path("expiresAt").asLong()?"approval_timeout":"access_revoked");r.clear();releaseAdmission(r);pendingOwners.remove(r.value.path("id").asText());}}}
    private void automaticStage(Request r,QueryJobs.Job job,String stage){
        if(job.cancelled)throw new java.util.concurrent.CancellationException();
        validate(r);job.progress=stage;
        agents.authorization.audit(r.principal,r.mcpSession,stage,r.value);
    }
    private void startAutomatic(Request r){
        try{
            validate(r);record(r,"auto_authorized","startup_yolo");r.state="submitted";
            ObjectNode job=jobs.local("agent:"+r.principal,r.value.path("type").asText().equals("native_command")?r.value.path("connectionId").asText():"",j->runAutomatic(r,j),()->{r.clear();pendingOwners.remove(r.value.path("id").asText());});
            r.jobId=job.path("id").asText();
        }catch(RuntimeException e){r.state="failed";r.clear();releaseAdmission(r);pendingOwners.remove(r.value.path("id").asText());throw e;}
    }
    private JsonNode runAutomatic(Request r,QueryJobs.Job job)throws Exception{
        automaticStage(r,job,"Validating exact administrative target");
        String type=r.value.path("type").asText();
        if(type.equals("native_command"))return nativeOperations.execute(job,r.value,r.payload,()->validate(r));
        if(type.equals("connection_test")){
            String id=r.value.path("target").path("connectionId").asText();
            ObjectNode input=Profiles.JSON.createObjectNode().put("connectionId",id);
            input.put("confirmDriverEffects",r.payload.path("confirmDriverEffects").asBoolean());
            ObjectNode result=setup.test("approval:"+r.value.path("id").asText(),input,job);result.remove("receipt");return result;
        }
        if(typeNeedsReceipt(r)){
            ObjectNode draft=(ObjectNode)r.payload.path("profile");
            if(r.value.path("requiresDriverInstall").asBoolean()){
                automaticStage(r,job,"Installing explicitly requested driver");
                ObjectNode installed=setup.bundles.install(r.value.path("driverInstall"),()->job.cancelled||!sessions.alive(r.mcpSession,r.principal),p->job.progress=p);
                DriverBundles.verify(installed);draft.set("jars",installed.path("jars").deepCopy());draft.set("driverBundle",installed.deepCopy());
                r.value.remove("requiresDriverInstall");
            }
            if(type.equals("connection_update"))draft.put("connectionId",r.value.path("target").path("connectionId").asText());
            if(r.payload.path("testBeforeSave").asBoolean()){
                automaticStage(r,job,"Testing unchanged connection draft");
                draft.put("confirmDriverEffects",r.payload.path("confirmDriverEffects").asBoolean());
                ObjectNode tested=setup.test("approval:"+r.value.path("id").asText(),draft,job);r.testReceipt=tested.path("receipt").asText();
            }
        }
        automaticStage(r,job,"Applying authorized configuration");
        JsonNode result=apply("startup_yolo",r,r.payload);
        record(r,"auto_applied","startup_yolo");
        return result;
    }
    private void record(Request r,String action,String session){try{if(Files.exists(audit)&&Files.size(audit)>4L<<20)Files.move(audit,audit.resolveSibling("agent-administration-approvals.previous.jsonl"),StandardCopyOption.REPLACE_EXISTING);if(!Files.exists(audit)){Files.createFile(audit);Profiles.protect(audit);}ObjectNode row=Profiles.JSON.createObjectNode().put("at",clock.getAsLong()).put("request",r.value.path("id").asText()).put("agent",r.principal).put("type",r.value.path("type").asText()).put("action",action).put("humanSession",session.equals("startup_yolo")?"":session).put("authorizationReason",r.value.path("authorizationReason").asText()).put("requestHash",r.hash);Files.writeString(audit,row+"\n",StandardCharsets.UTF_8,StandardOpenOption.APPEND);}catch(Exception e){throw new IllegalStateException("Approval audit could not be written; operation is not authorized",e);}}
    ...
    public synchronized void close(){requests.values().forEach(r->{r.clear();releaseAdmission(r);});requests.clear();ledger.clear();retainedCount=0;pendingOwners.clear();}
}
