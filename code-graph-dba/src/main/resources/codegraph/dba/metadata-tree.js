...
export class MetadataTreeView {
  ...
  async fetch(entry,offset=0){return this.wait(await this.api('/metadata/tree','POST',{connectionId:entry.profile.id,...entry.descriptor,offset}));}
  ...
  async reload(entry){
    ...
    try{
      ...
      this.footer(entry,result?.nextOffset);
