...
import {bindTreeContextMenu,placeTreeRows} from './tree-actions.js';
...
const sameShape=(child,node)=>child.descriptor.kind===node.kind&&!!child.descriptor.branch===!!node.branch;
const profileTarget=({name,color,connectionState,order,...target})=>JSON.stringify(target);
...
export class MetadataTreeView {
  ...
  invalidate(entry){entry.loaded=false;entry.revision=(entry.revision??0)+1;for(const operation of entry.operations??[])this.cancelLoad(operation);for(const child of entry.children??[])this.invalidate(child);}
  async load(profile,container){
    let root=this.roots.get(profile.id);
    if(root&&root.container!==container){this.invalidate(root);root=null;}
    if(!root){root={profile,key:profile.id,descriptor:{kind:'root'},container,children:[],revision:0,root:true};this.roots.set(profile.id,root);}
    else if(profileTarget(root.profile)!==profileTarget(profile)){this.invalidate(root);await root.refresh;}
    if(this.roots.get(profile.id)!==root||!container.isConnected)return false;
    this.updateProfile(root,profile);return this.reload(root);
  }
  updateProfile(entry,profile){entry.profile=profile;for(const child of entry.children)this.updateProfile(child,profile);}
  ...
  checkCapacity(entry,nodes,replacing=false){
    const incoming=new Map(nodes.map(node=>[node.key,node]));
    const retained=replacing?entry.children.filter(child=>incoming.has(child.descriptor.key)&&sameShape(child,incoming.get(child.descriptor.key))):[];
    const count=this.total()-(replacing?this.count(entry):0)+nodes.length+retained.reduce((n,child)=>n+this.count(child),0);
    const bytes=[...this.roots.values()].reduce((n,root)=>n+this.retainedBytes(root),0)-(replacing?this.retainedBytes(entry):0)+nodes.reduce((n,node)=>n+descriptorBytes(node),0)+retained.reduce((n,child)=>n+this.retainedBytes(child),0);
  ...
  sortKeys(entry){if(!isKeyScan(entry))return;entry.children.sort((a,b)=>a.descriptor.name.localeCompare(b.descriptor.name,undefined,{sensitivity:'base'})||a.descriptor.key.localeCompare(b.descriptor.key));this.order(entry);}
  scanControls(entry){
    if(!isKeyScan(entry)||entry.container.querySelector(':scope > .metadata-scan-controls'))return;
  ...
  reload(entry){
    ...
    entry.refresh=this.refreshEntry(entry).finally(()=>{entry.refresh=null;});return entry.refresh;
  }
  async refreshEntry(entry){
    entry.loading=true;const revision=++entry.revision;entry.wrapper?.setAttribute('aria-busy','true');
    // Read all previously loaded pages before reconciling, so a failed refresh keeps old children.
    ...
    try{
      ...
      nodes=[...new Map(nodes.map(node=>[node.key,this.normalize(entry,node)])).values()];this.checkCapacity(entry,nodes,true);
      this.reconcile(entry,nodes);entry.loaded=true;
      entry.container.querySelector(':scope > .metadata-error')?.remove();
      this.scanControls(entry);this.sortKeys(entry);this.footer(entry,result);if(isKeyScan(entry))this.pages.delete(entry.key);
      // Expanded descendants stay visible while fresh metadata is fetched. Collapsed
      // branches keep their cached DOM and are refreshed when next opened.
      for(const child of entry.children){
        if(entry.revision!==revision||!entry.container.isConnected)break;
        if(!child.descriptor.branch)continue;
        child.loaded=false;
        if(this.expanded.has(child.key)&&!entry.container.hidden){
          await child.refresh;
          if(entry.revision!==revision||!child.wrapper.isConnected)break;
          if(this.expanded.has(child.key)&&!entry.container.hidden)await this.toggle(child,true);
        }
      }
      return true;
  ...
  normalize(parent,node){
    if(parent.descriptor.kind==='tables')return {...node,relationType:'table'};
    const previous=node.kind==='native_keys'?parent.children.find(child=>child.descriptor.key===node.key):null;
    return previous?.descriptor.pattern!==undefined?{...node,pattern:previous.descriptor.pattern}:node;
  }
  order(entry){const controls=entry.container.querySelector(':scope > .metadata-scan-controls');placeTreeRows(entry.container,entry.children.map(child=>child.wrapper),controls?controls.nextElementSibling:entry.container.firstElementChild);}
  forget(entry){this.invalidate(entry);for(const state of [this.expanded,this.pages])for(const key of state.keys())if(key===entry.key||key.startsWith(entry.key+'/'))state.delete(key);entry.wrapper.remove();}
  reconcile(entry,nodes){
    const previous=new Map(entry.children.map(child=>[child.descriptor.key,child])),next=[];
    const focused=document.activeElement;
    for(const descriptor of nodes){
      let child=previous.get(descriptor.key);previous.delete(descriptor.key);
      if(child&&!sameShape(child,descriptor)){this.forget(child);child=null;}
      if(child){
        // Handlers retain this descriptor object, so update it rather than capturing
        // obsolete names, object capabilities or SQL targets after a refresh.
        if(JSON.stringify(child.descriptor)!==JSON.stringify(descriptor)){
          this.invalidate(child);
          for(const key of Object.keys(child.descriptor))delete child.descriptor[key];
          Object.assign(child.descriptor,descriptor);
          this.updateLabel(child);
        }
        this.updateProfile(child,entry.profile);
      }else child=this.append(entry,descriptor);
      next.push(child);
    }
    for(const child of previous.values())this.forget(child);
    entry.children=next;this.order(entry);
    if(focused&&!focused.isConnected)(entry.wrapper?.querySelector('.metadata-name')??entry.container.closest('.connection-row')?.querySelector('.connection-select'))?.focus({preventScroll:true});
  }
  updateLabel(entry){
    const {descriptor,wrapper,toggle}=entry,name=wrapper.querySelector('.metadata-name'),more=wrapper.querySelector('.metadata-actions');
    if(name.textContent!==descriptor.name)name.textContent=descriptor.name;
    wrapper.dataset.name=descriptor.name;wrapper.setAttribute('aria-label',descriptor.name);
    name.title=descriptor.name+(this.canOpen(entry)?' · Double-click to open object (Enter)':'');
    more.title='Actions for '+descriptor.name;more.setAttribute('aria-label',more.title);
    if(toggle){toggle.title=(entry.container.hidden?'Expand ':'Collapse ')+descriptor.name;toggle.setAttribute('aria-label',toggle.title);}
  }
  canOpen(entry){return entry.descriptor.nativeObject||objectParents.has(entry.parent.descriptor.kind)&&entry.parent.descriptor.kind!=='relation';}
  async restore(entry){for(const child of entry.children){if(!entry.container.isConnected||entry.container.hidden)return;if(child.descriptor.branch&&this.expanded.has(child.key))await this.toggle(child,true);}}
  footer(entry,result={}){
    const signature=JSON.stringify([result.nextOffset,result.scanComplete,result.warning,entry.descriptor.schedulerMessage,entry.children.length]);
    if(entry.footerSignature===signature)return;entry.footerSignature=signature;
    ...
    more.onclick=async()=>{
      ...
      try{
        ...
        const nodes=[...new Map([...entry.children.map(child=>child.descriptor),...this.scanNodes(entry,result.nodes).map(node=>this.normalize(entry,node))].map(node=>[node.key,node])).values()];
        this.checkCapacity(entry,nodes,true);this.reconcile(entry,nodes);
  ...
  append(parent,descriptor){
    descriptor=this.normalize(parent,descriptor);
