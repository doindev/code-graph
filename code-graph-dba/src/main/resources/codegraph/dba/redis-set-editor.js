...
export function redisMemberSnapshot(result,sorted=false){
  ...
  if(sorted&&value!==null&&(typeof value!=='number'||!Number.isFinite(value)))throw Error('Choose a finite score or an absent member. Infinite scores and incomplete values cannot be edited.');
  ...
  return{present:sorted?value!==null:value,...(sorted?{score:value}:{}),count,ttl};
}
...
export class RedisMemberEditor{
  constructor({run,readOnly,changed,close,key='',member='',sorted=false}){
    ...
    this.root=node('section');this.root.className='redis-string-editor';this.root.setAttribute('aria-label',sorted?'Redis sorted-set member editor':'Redis set member editor');
    const header=node('div');header.className='redis-string-actions';header.append(node('strong',sorted?'Sorted-set member · 8 KiB maximum':'Set member · 8 KiB maximum'));
    ...
    this.root.append(node('p',sorted?'Edit, explicitly add or delete one member in an existing sorted set; key/member up to 8 KiB. Redis binary64 precision, not arbitrary decimal precision. GEO indexes also use zset: TYPE does not prove score semantics. Updates/additions preserve key TTL but may change rank. Deleting the last member removes its key and TTL. No key creation, rename or automatic retries.':'One member in an existing set · current membership WATCH checks, not change history. Save preserves key TTL unless deletion removes the last member and its key. No set inventory, renaming or automatic retries.'));
    ...
    this.saveButton=this.button(footer,sorted?'Save sorted-set member':'Save set member','save',()=>this.save());this.revertButton=this.button(footer,sorted?'Revert sorted-set draft':'Revert membership draft','undo-2',()=>this.revert());
    this.status=node('span',sorted?'Enter a member (empty is valid), then load its score or establish absence. Nothing has been written.':'Enter a member (empty is valid), then check its membership. Nothing has been written.');this.status.setAttribute('role','status');footer.append(this.status);this.root.append(footer);this.sync();
  }
  ...
  get dirty(){if(!this.snapshot)return false;if(this.desired!==this.snapshot.present)return true;if(!this.sorted)return false;if(!this.snapshot.present)return this.score.value!=='';try{return redisScore(this.score.value)!==this.snapshot.score;}catch{return true;}}
  get pendingDeletion(){return !!this.snapshot?.present&&!this.desired;}
  get savable(){return this.dirty&&(!this.sorted||this.pendingDeletion||this.desired);}
  ...
  sync(){
    ...
    this.score.disabled=blocked||this.uncertain||!this.snapshot||this.pendingDeletion||this.readOnly();this.score.setAttribute('aria-invalid',String(!!this.snapshot&&!this.pendingDeletion&&!validScore));
    ...
    this.stageButton.replaceChildren(lucide(deleting?'trash-2':'plus'),document.createTextNode(label));this.stageButton.title=label;this.stageButton.setAttribute('aria-label',label);this.stageButton.setAttribute('aria-pressed',String(!!this.snapshot&&this.desired!==this.snapshot.present));
    this.saveButton.disabled=blocked||this.uncertain||!this.savable||(!this.pendingDeletion&&!validScore)||this.readOnly();this.revertButton.disabled=blocked||this.uncertain||!this.dirty;
    this.root.classList.toggle('redis-pending-delete',this.pendingDeletion);this.root.setAttribute('aria-busy',String(this.busy));
  }
  changeMode(){
  ...
  stage(){if(this.busy||this.disposed||this.unavailable||this.uncertain||!this.snapshot||this.readOnly())return;this.desired=!this.desired;this.status.textContent=this.desired!==this.snapshot.present?(this.desired?'Insertion staged; provide a valid score when required. Save opens review.':'Deletion staged; deleting the last member removes its set key and TTL. Save opens destructive review.'):'Membership change unstaged; nothing was written.';this.sync();this.changed();}
  revert(){if(this.busy||this.disposed||this.uncertain||!this.snapshot)return;this.desired=this.snapshot.present;if(this.sorted)this.score.value=this.snapshot.present?String(this.snapshot.score):'';this.status.textContent='Draft reverted; nothing was written.';this.sync();this.changed();}
  ...
  async load(){
    ...
    try{
      ...
      const snapshot=redisMemberSnapshot(reply.result,this.sorted);this.snapshot={...snapshot,key,member,targetRevision:reply.targetRevision};this.desired=snapshot.present;this.uncertain=false;if(this.sorted)this.score.value=snapshot.present?String(snapshot.score):'';
      this.status.textContent=(this.sorted&&snapshot.present?'Score loaded: '+snapshot.score:snapshot.present?'Member present':'Member absent; insertion must be staged explicitly')+' · '+snapshot.count+' member(s) at read · '+(snapshot.ttl===-1?'no expiry':'TTL '+snapshot.ttl+' ms')+'. Reads are not a frozen snapshot; Save rechecks current state.';
  ...
  async save(){
    if(this.busy||this.disposed||this.unavailable||this.uncertain||!this.savable||this.readOnly())return;
    const snapshot=this.snapshot,desired=this.desired;let score;try{if(this.sorted&&desired)score=redisScore(this.score.value);}catch(e){this.status.textContent=e.message;return;}
    const command=this.sorted?(desired?['ZADD',snapshot.key,String(score),snapshot.member]:['ZREM',snapshot.key,snapshot.member]):[desired?'SADD':'SREM',snapshot.key,snapshot.member];
    const watch=this.sorted?{key:snapshot.key,scoreMember:snapshot.member,expected:snapshot.present?String(snapshot.score):null}:{key:snapshot.key,member:snapshot.member,expected:snapshot.present};
    const expectedReply=this.sorted&&desired&&snapshot.present?0:1;
    this.busy=true;this.sync();
    try{
      ...
      if(result?.outcome==='acknowledged'&&result.entries.length===1&&entry?.index===0&&entry.state==='acknowledged'&&entry.value===expectedReply){
        this.snapshot=desired?{...snapshot,present:true,...(this.sorted?{score}:{})}:null;this.desired=desired;this.uncertain=false;if(!desired)this.score.value='';
        this.status.textContent=desired?(this.sorted&&snapshot.present?'Score updated':'Member added')+' · key TTL preserved. Reload to observe subsequent changes.':'Member deleted · if it was the last member, Redis removed the set key and TTL. Nothing was recreated.';
