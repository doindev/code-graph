...
export function installOracleAdministration({api,profiles,notify}) {
  ...
  const main=el('main');main.append(tools,catalog,operation,review,last);const layout=el('div',undefined,'oracle-admin-layout');layout.append(nav,main);
  const footer=el('footer');footer.append(status,cancel);dialog.append(header,target,error,layout,footer);
  let selected=null,capabilities=null,category='users',offset=0,pageSize=100,hasMore=false,busy=false,closing=false,currentJob=null,reviewJob=null,plan=null,selectedRow=null;
  function update(){
  ...
  async function requestClose(){closing=true;if(busy){await cancelJob();return;}await releaseReview();dialog.close();}
  ...
  async function open(){if(busy)return;closing=false;await releaseReview();const existing=connection.value;connection.replaceChildren();
    ...
    selected=null;capabilities=null;nav.replaceChildren();catalog.replaceChildren();form.replaceChildren();actions.replaceChildren();last.hidden=true;error.textContent='';status.textContent='';target.textContent=connection.options.length?'Choose Connect to inspect the resolved Oracle target.':'Add a native Oracle connection to use Administration.';update();dialog.showModal();
  }
  connection.onchange=()=>{selected=null;capabilities=null;nav.replaceChildren();catalog.replaceChildren();form.replaceChildren();actions.replaceChildren();target.textContent='Connect to inspect this Oracle target.';releaseReview();};
  async function connect(){await releaseReview();selected={connectionId:connection.value,database:''};capabilities=null;catalog.replaceChildren();nav.replaceChildren();
    ...
    for(const item of capabilities.categories){const b=button(item.label,()=>act(async()=>{await releaseReview();category=item.id;filter.value='';selectedRow=null;renderActions();await read(0);}));b.dataset.category=item.id;nav.append(b);}
    category='users';filter.value='';renderActions();await read(0);
  }
  ...
  function renderFields(){form.replaceChildren();const action=capabilities?.actions.find(a=>a.id===actions.value);if(!action)return;
    const row=selectedRow??{},values={name:row.username??row.role??row.profile??row.tablespace_name??row.table_name??row.name,owner:row.owner??capabilities.target.schema,tablespace:row.tablespace_name,fileId:row.file_id,kind:row.kind,sid:row.sid,serial:row['serial#'],sqlId:row.sql_id,object:row.object_name,objectType:row.type};
  ...
  async function applyPlan(){const id=reviewJob,secret=password.value;password.value='';let result;
    try{result=await job('/oracle/admin/apply',{planId:id,confirmed:ack.checked,...(plan.requiresPassword?{password:secret}:{})},{mutation:true});last.hidden=false;if(result.status!=='success')error.textContent=result.message;else notify(result.message);}finally{last.hidden=false;await releaseReview();main.scrollTop=0;}
  }
  update();
}
