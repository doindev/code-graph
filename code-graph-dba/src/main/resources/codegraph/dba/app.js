...
window.addEventListener('pagehide',()=>{
  const ids=new Set();for(const tab of tabs){for(const result of tab.result?.results??(tab.result?[tab.result]:[]))if(result.grid?.id)ids.add(result.grid.id);if(tab.builder?.result?.grid?.id)ids.add(tab.builder.result.grid.id);tab.designer?.dispose();tab.native?.dispose();tab.builder?.dispose().catch(()=>{});}
...
window.addEventListener('beforeunload',e=>{if(tabs.some(t=>t.dirty||(t.result?.results??(t.result?[t.result]:[])).some(DataGridView.dirty)||t.builder?.rowsDirty)){e.preventDefault();e.returnValue='';}});
