...
const isScript=tab=>!!tab&&!['table','builder','object','native'].includes(tab.type);
function nativeFor(tab){
  ...
  }});tab.native.profile=profile;return tab.native;
}
...
window.addEventListener('pagehide',()=>{for(const tab of tabs){tab.designer?.dispose();tab.native?.dispose();tab.builder?.dispose().catch(()=>{});}for(const schedule of gridRefreshers.values())schedule.dispose().catch(()=>{});gridRefreshers.clear();});
...
function renderDocument(){
  pruneGridRefreshers();for(const open of tabs){open.builder?.unmount();open.native?.unmount();}
