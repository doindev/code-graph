...
let csrf='',profiles=[],tabs=[],active=null,lastSelected=null,toastTimer,workspaceTimer=0,workspaceReady=false,workspaceSaving=false,workspaceQueued=false,lastWorkspaceJson='';
const MAX_TABS=12,MAX_BROWSER_BYTES=32*1024*1024;
...
const isScript=tab=>!!tab&&!['table','builder','object'].includes(tab.type);
function builderFor(tab){if(!tab.builder)tab.builder=new VisualQueryBuilder({api,initialDirty:!!tab.dirty,connectionId:tab.connection,connectionName:profiles.find(p=>p.id===tab.connection)?.name??'Unavailable',database:tab.database??tab.table?.parent.database??'',selection:tab.table??null,sql:tab.type==='builder'?tab.sql:'',draft:tab.draft??tab.builderDraft,openScript:(sql,database)=>{const script=openTab(tab.connection,'Query.sql');script.sql=sql;script.dirty=true;renderDocument();},changed:builder=>{if(tab.type==='builder'){tab.sql=builder.sql;tab.database=builder.database;}tab.dirty=builder.dirty||!!tab.designer?.dirty;renderTabs();},openBuilder:(sql,database)=>openBuilder(tab.connection,sql,database),account:bytes=>{if(tabs.reduce((sum,t)=>sum+(t===tab?0:(t.bytes??0)+(t.table?t.builderBytes??0:0)),0)+bytes+(tab.table?tab.bytes??0:0)>MAX_BROWSER_BYTES)throw new Error('Browser result allowance full; close a result and retry.');tab.builderBytes=bytes;if(tab.type==='builder')tab.bytes=bytes;}});tab.builder.connectionName=profiles.find(p=>p.id===tab.connection)?.name??'Unavailable';return tab.builder;}
...
function openObject(profile,descriptor,selection){
...
function objectDesigner(tab){
  if(!tab.designer)tab.designer=new ObjectProperties({
    ...
    changed:()=>{tab.dirty=!!tab.designer?.dirty;renderTabs();},
...
function renderTableDocument(tab){
  ...
  for(const [index,key] of ['properties','data','diagram'].entries()){
    const unavailable=!!tab.creating&&key!=='properties';const control=button(key[0].toUpperCase()+key.slice(1),'Show '+key,()=>{tab.inner=key;renderTableDocument(tab);queueWorkspaceSave();tableDocument.querySelector('[aria-selected=true]').focus();});
    control.disabled=unavailable;if(unavailable)control.title='Create the table successfully before opening '+key;
    ...
    control.addEventListener('keydown',event=>{if(tab.creating){event.preventDefault();return;}const keys=['properties','data','diagram'];let next;if(event.key==='ArrowRight')next=(index+1)%3;else if(event.key==='ArrowLeft')next=(index+2)%3;else if(event.key==='Home')next=0;else if(event.key==='End')next=2;else return;event.preventDefault();tab.inner=keys[next];renderTableDocument(tab);queueWorkspaceSave();tableDocument.querySelector('[aria-selected=true]').focus();});nav.append(control);
  }
  ...
  if(tab.inner==='diagram'){builderFor(tab).mount(panel);return;}
  ...
  activeGrid=new DataGridView(host,result,{sourceSql:context?.previewSql??context?.displaySql??context?.sql,notice:toast,transform:change=>controller.execute(change),refresh:controller.refresh,controller});
...
async function loadTableData(tab,result,execution){
  ...
  const response=await execution.submit({connectionId:tab.connection,database:prepared.database,sql:prepared.sql,parameters:[],autoCommit:false});execution.check();
...
function stopGridRefresh(result){const done=gridRefreshers.get(result)?.dispose()??Promise.resolve();gridRefreshers.delete(result);done.catch(()=>{});return done;}
...
function pruneGridRefreshers(){const retained=new Set(tabs.flatMap(tab=>tab.result?.results??(tab.result?[tab.result]:[])));for(const result of gridRefreshers.keys())if(!retained.has(result))stopGridRefresh(result);}
function gridControllerFor(tab,result){if(tab.closing||tab.job)return null;let controller=gridRefreshers.get(result);if(!controller){controller=new GridQueryController({api,run:(change,execution)=>result.tablePending?loadTableData(tab,result,execution):transformGrid(tab,result,change,execution),canRun:automatic=>!tab.closing&&!tab.job&&!(automatic&&(document.hidden||document.querySelector('dialog[open]')||document.activeElement?.matches('input,textarea,[contenteditable=true]')))});gridRefreshers.set(result,controller);}return controller;}
window.addEventListener('pagehide',()=>{for(const tab of tabs){tab.designer?.dispose();tab.builder?.dispose().catch(()=>{});}for(const schedule of gridRefreshers.values())schedule.dispose().catch(()=>{});gridRefreshers.clear();});
...
async function api(path,method='GET',body){const response=await fetch('/api/dba'+path,{method,credentials:'same-origin',headers:{'Content-Type':'application/json','X-Dba-CSRF':csrf},body:body===undefined?undefined:JSON.stringify(body)});const data=await response.json();if(!response.ok){if(response.status===403&&data.error==='DBA session expired'){const session=await api('/bootstrap','POST',{});csrf=session.csrf;throw new Error('Session renewed. Please retry your action; active tests must be repeated.');}const failure=new Error(data.error||'Request failed');failure.status=response.status;throw failure;}return data;}
...
async function flushWorkspace(){clearTimeout(workspaceTimer);workspaceTimer=0;if(!workspaceReady||!csrf)return;if(workspaceSaving){workspaceQueued=true;return;}const state=workspaceState(),json=JSON.stringify(state);if(json===lastWorkspaceJson)return;if(new TextEncoder().encode(json).length>MAX_WORKSPACE_BYTES){toast('Open Script state exceeds the 16 MiB session workspace limit. Save or close a large Script to resume recovery updates.');return;}workspaceSaving=true;try{await api('/workspace','PUT',state);lastWorkspaceJson=json;}catch(e){console.warn('Workspace recovery update failed:',e.message);workspaceQueued=true;}finally{workspaceSaving=false;if(workspaceQueued){workspaceQueued=false;queueWorkspaceSave(1000);}}}
async function restoreWorkspace(){const saved=await api('/workspace');const restored=Array.isArray(saved.tabs)?saved.tabs.slice(0,MAX_TABS):[];tabs=restored.map(item=>item.type==='object'?{...item,result:null,bytes:0,job:null,dirty:false}:item.type==='builder'?{...item,result:null,bytes:0,job:null}:item.type==='table'?newTableState(item):({id:item.id,title:item.title,lastEdited:item.lastEdited??0,connection:item.connection??null,sql:item.sql??'',savedText:item.dirty?null:(item.sql??''),dirty:!!item.dirty,fileHandle:null,params:'[]',autoCommit:false,result:null,resultIndex:0,outputView:item.outputView??null,error:'',warning:'',job:null,bytes:0,editorRatio:Math.max(.1,Math.min(.9,Number(item.editorRatio)||.38)),railToggles:{serverOutput:!!item.railToggles?.serverOutput,executionLog:!!item.railToggles?.executionLog,sqlVariables:!!item.railToggles?.sqlVariables}}));active=tabs.some(tab=>tab.id===saved.active)?saved.active:(tabs.at(-1)?.id??null);if(profiles.some(profile=>profile.id===saved.lastSelected))lastSelected=saved.lastSelected;renderTabs();renderDocument();workspaceReady=true;lastWorkspaceJson=JSON.stringify(workspaceState());if(active&&isScript(tabs.find(t=>t.id===active)))requestAnimationFrame(()=>$('sql').focus());}
async function initialize(){workspaceReady=false;const session=await api('/bootstrap','POST',{});csrf=session.csrf;await refresh();await restoreWorkspace();}
...
const connectionTree=new ConnectionTree($('tree'),{api,notice:toast,select:chooseConnection,load:loadSchemas,wait:waitJob,edit:profileDialog,
  open:id=>{chooseConnection(id);return openScript();},new:id=>openTab(id),
...
function openTab(connection=lastSelected,title='Untitled.sql'){saveEditor();if(tabs.length>=MAX_TABS)throw new Error('Close a tab before opening another (maximum 12).');const tab={type:'script',id:crypto.randomUUID(),lastEdited:Date.now(),title,connection,sql:'',savedText:'',dirty:false,fileHandle:null,params:'[]',autoCommit:false,result:null,resultIndex:0,outputView:null,error:'',warning:'',job:null,bytes:0,editorRatio:.38,railToggles:{serverOutput:false,executionLog:false,sqlVariables:false}};tabs.push(tab);active=tab.id;renderTabs();renderDocument();return tab;}
const newBuilder=button('','New Visual Query Builder',()=>openBuilder());newBuilder.id='new-query-builder';newBuilder.setAttribute('aria-label','New Visual Query Builder');newBuilder.append(lucide('workflow'));$('file-tools').after(newBuilder);
const selectionBuilder=button('','Open selected SQL in Visual Query Builder',()=>{saveEditor();const tab=tabs.find(t=>t.id===active);if(!isScript(tab))return;const sql=$('sql').value.substring($('sql').selectionStart,$('sql').selectionEnd);if(!sql.trim())throw new Error('Select one SELECT query in the Script editor first.');openBuilder(tab.connection,sql);});selectionBuilder.id='selection-query-builder';selectionBuilder.setAttribute('aria-label','Open selected SQL in Visual Query Builder');selectionBuilder.append(lucide('workflow'));$('stop').after(selectionBuilder);
...
function renderTabs(){
  ...
  for(const tab of tabs){
    ...
    const connection=profiles.find(p=>p.id===tab.connection)?.name??'Not connected';
    ...
    target.addEventListener('change',safe(async()=>{
      if(tab.job)return;saveEditor();const nextConnection=target.value||null;tab.job='submitting';renderDocument();try{await stopScriptRefreshes(tab);}catch(error){tab.error=error.message;return;}finally{tab.job=null;renderDocument();}if(!tabs.includes(tab))return;active=tab.id;tab.connection=nextConnection;chooseConnection(tab.connection);
...
async function transformGrid(tab,result,change,execution){
  ...
  const updateView=()=>{if(activeGrid?.result===result){const current=gridContexts.get(result);activeGrid.updateData(current?.previewSql??current?.displaySql??current?.sql);}};
  try{
    ...
    Object.assign(result,replacement);DataGridView.clearSelection(result);gridContexts.set(result,{...context,...edited});
...
settingsToggle.append(lucide('settings'));$('settings').prepend(lucide('cpu'));$('agents').prepend(lucide('key-round'));
...
for(const id of ['settings-dialog','agents-dialog'])$(id).addEventListener('close',()=>{const focused=document.activeElement;if(!focused||focused===document.body||settingsMenu.contains(focused)||$(id).contains(focused))settingsToggle.focus();});
$('settings').onclick=safe(async()=>{const s=await api('/settings'),f=$('settings-form');f.elements.memory.value=(s.memoryBudget/1048576)+'m';for(const k of ['concurrency','uiRows','agentRows','timeoutSeconds','decisionTimeoutSeconds'])f.elements[k].value=s[k];$('telemetry').textContent=JSON.stringify(s,null,2);$('settings-dialog').showModal();});
$('settings-form').addEventListener('submit',safe(async e=>{e.preventDefault();const b=Object.fromEntries(new FormData(e.target));for(const k of ['concurrency','uiRows','agentRows','timeoutSeconds','decisionTimeoutSeconds'])b[k]=Number(b[k]);await api('/settings','PUT',b);$('settings-dialog').close();toast('DBA limits updated for this run.');}));
...
initialize().catch(e=>toast(e.message));
...
function renderResults(tab){
  ...
  rowResults.forEach((result,index)=>{const label='Result '+(index+1);resultTabs.append(outputTab(label,'Show SQL result '+(index+1),tab.outputView==='result'&&index===tab.resultIndex,()=>{tab.outputView='result';tab.resultIndex=index;renderResults(tab);},()=>{stopGridRefresh(result);const current=tab.resultIndex??0,actualIndex=results?.indexOf(result)??-1;if(actualIndex>=0)results.splice(actualIndex,1);else if(tab.result===result)tab.result=null;const remainingRows=results?.filter(item=>item.kind==='rows')??[];if(!results?.length){tab.result=null;tab.resultIndex=0;tab.resultConnection=null;tab.bytes=0;}else{tab.resultIndex=index<current?current-1:Math.min(current,Math.max(0,remainingRows.length-1));tab.result.rowCount=remainingRows.reduce((count,item)=>count+(item.rows?.length??0),0);const affected=results.filter(item=>item.affectedRows!==undefined);if(affected.length)tab.result.affectedRows=affected.reduce((count,item)=>count+item.affectedRows,0);else delete tab.result.affectedRows;tab.bytes=new TextEncoder().encode(JSON.stringify(tab.result)).length*3;}if(tab.outputView==='result')tab.outputView=fallbackOutputView(tab,remainingRows);renderDocument();}));});
...
document.addEventListener('keydown',e=>{if(document.querySelector('dialog[open]')||!(e.ctrlKey||e.metaKey))return;const key=e.key.toLowerCase();if(key==='s'){e.preventDefault();const tab=tabs.find(t=>t.id===active);safe(()=>tab?.builder?tab.builder.saveFile(e.shiftKey):saveScript(e.shiftKey))();}else if(key==='o'){e.preventDefault();safe(openScript)();}else if(key==='n'&&e.altKey){e.preventDefault();safe(()=>openTab(lastSelected))();}else if(e.key==='Enter'){e.preventDefault();const tab=tabs.find(t=>t.id===active);if(tab?.builder)void tab.builder.run('data');else $('run').click();}});
...
window.addEventListener('pagehide',()=>{if(!workspaceReady||!csrf)return;const json=JSON.stringify(workspaceState());if(json===lastWorkspaceJson)return;const bytes=new TextEncoder().encode(json);if(bytes.length<=60*1024)fetch('/api/dba/workspace',{method:'PUT',credentials:'same-origin',keepalive:true,headers:{'Content-Type':'application/json','X-Dba-CSRF':csrf},body:json}).catch(()=>{});});
