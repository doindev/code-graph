...
async function loadSchemas(p,container){return metadataTree.load(p,container);}
...
function renderResults(tab){
  ...
  if(tab.outputView&&tab.outputView!=='result'){renderScriptOutput(tab);return;}
...
function renderScriptOutput(tab){
  destroyGrid();const host=$('grid');host.className='script-output';host.setAttribute('aria-label',RAIL_TOGGLES.find(([,key])=>key===tab.outputView)?.[2]??'Output');
  const append=(tag,text)=>{const item=document.createElement(tag);item.textContent=text;host.append(item);return item;};
  if(tab.outputView==='serverOutput'){
    const output=tab.result?.serverOutput;append('p',output?'Oracle DBMS_OUTPUT':'Enable this tab before running Oracle SQL to collect DBMS_OUTPUT.');
    if(output){append('pre',(output.lines??[]).join('\n')||'No server output.');if(output.truncated)append('p','Output was truncated at 256 lines or 100,000 bytes.');if(output.message)append('p',output.message);}return;
  }
  ...
  append('p',tab.result?.message||'No execution has completed.');if(tab.result?.transactionNotice)append('p',tab.result.transactionNotice);
  for(const unit of tab.result?.statements??[]){append('h3','Statement '+unit.index);append('pre',unit.sql);for(const error of tab.result?.errors??[])if(error.statementIndex===unit.index)append('p',error.message+' · '+(error.resolution||'stopped'));}
}
