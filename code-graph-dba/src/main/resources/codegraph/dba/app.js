...
function renderTableDocument(tab){
  ...
  activeGrid=new DataGridView(host,result,{rowActions:!isView(tab),sourceSql:context?.previewSql??context?.displaySql??context?.sql,notice:toast,transform:change=>controller.execute(change),refresh:controller.refresh,controller});
