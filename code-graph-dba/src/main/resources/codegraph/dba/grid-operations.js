import {preferences,pagePreference} from './grid-preferences.js';
...
export async function runOwnedGrid(result,change,execution,{api,replace,download=downloadExport}) {
  const settings=preferences(result);execution.cancellationTimeoutMs=settings.cancelTimeout;const context=result.grid;if(!context)throw Error('Run this query again to establish a grid context.');
  ...
  if(change.action==='count'){
    const counted=await submit('count',{});if(counted.revision!==result.grid.revision)return false;
    result.grid.page.total=counted.total;result.grid.page.totalCapturedAt=counted.capturedAt;await replace(result);return true;
  }
  if(change.action==='reconcile'){
    ...
    const next=await submit('reconcile',{direction:'refresh',limit:pagePreference(result),countPolicy:settings.autoCount?'auto':'none'});
    DataGridView.cancelDraft(result);await replace(next);
    return true;
  }
  if(change.action==='save_rows'){
    const draft=DataGridView.draft(result);if(!draft.dirty)return true;if(!draft.capabilities.edit)throw Error(draft.capabilities.reason||'This result is read-only.');
    ...
    if((plan.deletes||settings.confirmSave)&&!await confirmDeletes(plan))throw Error('Save cancelled; staged changes are retained.');
    ...
    result.grid.revision=saved.revision;DataGridView.freezeSaved(result);execution.stopRefresh?.();
    if(!settings.refreshAfterSave){await replace(result);return true;}
    try{execution.check();await replace(await submit(context.capabilities.page?'page':'reload',{direction:'refresh',limit:pagePreference(result),countPolicy:settings.autoCount?'auto':'none'}));
    ...
    catch(error){await replace(result);throw Error('Saved successfully, but reload failed. Do not repeat the save: '+error.message);}
    return true;
  }
  if(change.action==='page'){
    ...
    await replace(await submit(kind,{direction:change.direction,limit:change.limit??pagePreference(result),countPolicy:settings.autoCount?'auto':'none',...(change.direction==='window'?{offset:change.offset}:{})}),{window:change.direction==='window'});
    return true;
  }
...
function confirmDeletes(plan){return new Promise(resolve=>{
  let approved=false;const dialog=gridDialog(plan.deletes?'Save row deletions?':'Save row changes?',({dialog,body,footer})=>{
