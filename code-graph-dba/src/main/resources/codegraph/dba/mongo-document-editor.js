...
export class MongoDocumentEditor{
  ...
  async load(){
    ...
    try{
      ...
      const reply=await this.run({find:this.collection,filter:{_id:{$eq:id}},limit:2},metadata.targetRevision);if(this.disposed)return;
      const documents=entries(reply);if(documents.length!==1)throw Error(documents.length?'Ambiguous document identity; no editable snapshot loaded.':'Document not found.');
