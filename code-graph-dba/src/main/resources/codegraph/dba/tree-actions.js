...
export class TreeActions {
  ...
  object(action,profile,selection,plan,refresh,before=async()=>{}){
    ...
    this.open({action,name:plan.name,context:profile.name+' · '+(plan.database||'')+' · '+plan.type+' '+plan.target,
      warning:action==='delete'?'This will delete the selected database object, including its data where applicable. It may commit immediately and cannot be undone here. No CASCADE or FORCE will be added. Database-owned dependencies may also be removed. '+(plan.deleteWarnings??[]).join(' '):action==='truncate'?'This removes ALL rows from the table and its partitions, where applicable, while retaining its structure. The database may reset identity values or commit immediately. There is no undo here. No CASCADE is added.':"Renaming may break SQL scripts or dependencies that reference the old name. "+plan.reason,
