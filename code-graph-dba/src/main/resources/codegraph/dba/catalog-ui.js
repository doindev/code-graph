...
export function installCatalogUI({api,profiles,menu}){
  ...
  statusButton.type=refresh.type='button';statusButton.prepend(lucide('info'));refresh.prepend(lucide('refresh-cw'));actions.append(statusButton,refresh);
  ...
  dialog.append(title,description,form,actions,status,error,searchForm,results,ddl,close);document.body.append(dialog);
  ...
  function showStatus(data){
    ...
    status.textContent=[data.state||'Snapshot',data.generation?'Generation '+data.generation:'',
      ...
      data.waitTimedOut?'Still pending; check Status again.':''].filter(Boolean).join(' · ');
  ...
  async function perform(operation,extra={}){
    ...
    try{
      ...
      showStatus(data);
      if(operation==='refresh_catalog'&&data.state==='scanning'){
        for(let attempt=0;attempt<6&&data.state==='scanning';attempt++){
          data=await api('/catalog','POST',{...scope,operation:'scan_status',afterGeneration:data.generation||0,waitMillis:5000});
  ...
  for(const node of [connection,database,schema])node.addEventListener('input',()=>{revision++;busy=false;results.replaceChildren();ddl.hidden=true;status.textContent='Target changed; check Status or Refresh catalog.';availability();});
