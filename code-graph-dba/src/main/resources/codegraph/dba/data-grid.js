import {preferences,preferenceState,pagePreference,rowHeight,formatCell,dateInputType} from './grid-preferences.js';
import {lucide} from './tree-icons.js';
import {GridDraft,pageSize} from './grid-state.js';
import {FloatingCellMode} from './grid-cell-editor.js';

import {GridValuePicker,compareValues,valueKey} from './grid-values.js';
import {GridSearch} from './grid-search.js';
import {GridScrollWindow,replaceGridPage} from './grid-window.js';
import {chooseDirty,gridSettings,exportDialog,gridDialog,action} from './grid-interactions.js';
const ROW_NUMBERS=Symbol('row numbers');
...
function stableColumns(result){const used=new Set();return(result.columns??[]).map((column,index)=>{let id=String(column.id??`c${index+1}`);while(used.has(id))id+='-'+(index+1);used.add(id);return{id,label:String(column.displayLabel??column.label??id),rawLabel:String(column.label??id),type:String(column.type??''),jdbcType:column.jdbcType??12,source:index,sourceName:column.table&&column.name?String(column.name):null};});}
function stateFor(result){let state=states.get(result);if(state)return state;const columns=stableColumns(result);state={columns,order:columns.map(c=>c.id),widths:new Map(columns.map(c=>[c.id,180])),hidden:new Set(),nullText:'NULL',dates:'database',draft:new GridDraft(result),filter:'',expanded:false,selectedRow:null,popup:{width:560,height:170}};state.draft.readOnly=()=>preferences(result).readOnly;state.draft.afterActive=()=>preferences(result).insertPosition==='active';states.set(result,state);return state;}

export class DataGridView {
  get preferences(){return preferences(this.result);}
  get rowHeight(){return rowHeight(this.result);}
  static preferences(result){return preferences(result);}
  static pendingPageSize(result){return pagePreference(result);}
  static bindPreferences(result,connectionId){preferenceState(result,connectionId);}
  static freezeSaved(result){
    const state=stateFor(result),draft=state.draft,columns=state.columns;
    const preview=Array.from({length:draft.length},(_,i)=>i).filter(i=>!draft.deleted(i)).map(i=>columns.map(c=>({...draft.cell(i,c)})));
    draft.cancel();draft.preview=preview;state.selectedRow=null;delete result.grid.page.total;delete result.grid.page.totalCapturedAt;result.grid.refreshRequired=true;
  }
  applyPreferences(){
    const p=this.preferences;this.state.nullText=p.nullText;this.state.dates=p.dates;
    if(this.controller)this.controller.cancellationTimeoutMs=p.cancelTimeout;
    this.state.valuePreferences?.clear();this.state.rowNumberDigits=null;
    if(p.sorting!=='loaded')this.state.localSort=null;
    if(this.pageInput)this.pageInput.value=String(pagePreference(this.result));
    this.renderTable();this.rowControls();this.scheduleCount();
  }
  scheduleCount(){
    const grid=this.result.grid;
    if(this.disposed||!this.preferences.autoCount||!grid?.capabilities.count||grid.refreshRequired||grid.uncertain||this.controller?.busy||!this.transform||grid.page?.total!==undefined||this.state.countAttempt===grid.id+':'+grid.revision)return;
    this.state.countAttempt=grid.id+':'+grid.revision;
    queueMicrotask(()=>{if(!this.disposed&&!this.controller?.busy&&!document.querySelector('dialog[open]'))void this.transform({action:'count',automatic:true}).catch(()=>{});else this.state.countAttempt=null;});
  }
  updateMatchingValues(){
    const draft=this.state.draft,activeColumn=this.state.columns.find(c=>c.id===draft.activeColumn),active=activeColumn&&draft.active!==null?draft.cell(draft.active,activeColumn):null;
    for(const cell of this.scroll.querySelectorAll('.grid-body [role=gridcell]')){
      const column=this.state.columns.find(c=>c.id===cell.dataset.columnId),value=draft.cell(Number(cell.closest('[data-row-index]').dataset.rowIndex),column);
      cell.classList.toggle('grid-value-match',!!(active&&activeColumn.jdbcType===column.jdbcType&&active.kind===value.kind&&valueKey(active.value??null,column.jdbcType)===valueKey(value.value??null,column.jdbcType)));
    }
  }
  displayedRows(){
    const draft=this.state.draft,all=Array.from({length:draft.length},(_,i)=>i);
    if(!draft.preview&&draft.placements?.size)for(const [id,anchor]of draft.placements){const at=all.findIndex(i=>draft.id(i)===id);if(at<0)continue;const [index]=all.splice(at,1),target=all.findIndex(i=>draft.id(i)===anchor);all.splice(target<0?all.length:target+1,0,index);}
    const filter=this.searchRows?new Set(this.searchRows):null,rows=filter?all.filter(i=>filter.has(i)):all;
    const sort=this.state.localSort,column=sort&&this.state.columns.find(c=>c.id===sort.id);
    if(column)rows.sort((a,b)=>{const x=draft.cell(a,column),y=draft.cell(b,column);return compareValues(x.kind==='null'?null:x.value??'',y.kind==='null'?null:y.value??'',column.jdbcType)*(sort.direction==='DESC'?-1:1);});
    return rows;
  }
  showError(message,automatic=false){
    if(!message)return;this.queryError.hidden=false;this.queryError.textContent=message;
    if(!automatic&&this.preferences.errors!=='inline'&&this.lastDialogError!==message){
      this.lastDialogError=message;gridDialog('Grid error',({body,footer,dialog})=>{body.textContent=message;action(footer,'Close',()=>dialog.close());});

    }
    if(!automatic&&this.preferences.errors==='dialog')this.queryError.textContent='Grid operation failed. See the error dialog for details.';
  }
  static replacePage(result,next,window=false){replaceGridPage(result,next,stateFor(result),window);}
  ...
  updateData(sourceSql){this.valuePicker?.contextChanged();const applied=this.result.grid?.page?.limit;if(applied!==undefined&&applied!==this.appliedPageSize){this.pageInput.value=String(applied);this.pageInput.setCustomValidity('');this.appliedPageSize=applied;}this.sourceSql=sourceSql;const preview=this.command.querySelector('.grid-source-preview');preview.value=sourceSql?.replace(/[\r\n]/g,' ').trim()||'SQL unavailable — JDBC metadata result';preview.title=preview.value;if(this.viewer)this.viewer.querySelector('textarea').value=sourceSql??'';if(this.filter)this.filter.value=this.state.filter;this.sourceButton.disabled=!sourceSql;this.syncColumns();const scroll=this.state.windowScroll?this.state.scroll:{left:this.scroll.scrollLeft,top:this.scroll.scrollTop};this.state.windowScroll=false;this.renderTable();if(scroll){this.scroll.scrollLeft=scroll.left;this.scroll.scrollTop=scroll.top;}this.windowLoader?.reset();this.paintRows?.();}
  ...
  renderQueryState(){
    ...
    if(query.busy)this.finishCell?.();
    this.host.setAttribute('aria-busy',String(query.busy));this.interactive.inert=query.busy&&!query.scrolling;this.command.inert=this.footer.inert=query.busy;this.rowControls();this.queryError.hidden=!query.error;this.queryError.textContent=query.error;if(query.error)this.showError(query.error,query.automatic);else this.lastDialogError=null;if(!query.busy)this.scheduleCount();this.find?.update();if(this.result.grid?.uncertain){const reconcile=document.createElement('button');reconcile.type='button';reconcile.textContent='Reconcile with database';reconcile.disabled=query.busy;reconcile.onclick=()=>void this.transform({action:'reconcile'}).catch(error=>this.notice(error.message));this.queryError.append(reconcile);this.queryError.hidden=false;}
    if(query.busy){
      if(!this.overlay){
        ...
        const overlay=document.createElement('div');overlay.className='grid-query-overlay'+(query.scrolling?' grid-scroll-progress':'');const panel=document.createElement('div');panel.className='grid-query-progress';
        ...
        if(!query.scrolling&&(focusedInside||!query.automatic&&document.activeElement===document.body))cancel.focus();
      }
  ...
  static setValueFilters(result,filters=[]){const state=stateFor(result);state.valueSelections=new Map(filters.map(filter=>[filter.columnId,filter.values]));}
  ...
  async saveRows(){if(!this.state.draft.dirty)return;if(!this.state.draft.capabilities.edit)throw Error(this.state.draft.capabilities.reason);if(!this.transform)throw Error('This result cannot save changes.');await this.transform({action:'save_rows',...this.state.draft.payload()});this.renderTable();if(this.state.draft.dirty)throw Error('Changes were not saved.');}
  openGridSettings(){gridSettings(this);}
  async exportData(){if(this.result.grid?.refreshRequired)throw Error('Refresh saved data before exporting this result.');if(!this.transform||!this.result.grid)throw Error('Refresh this result to establish an export context.');if(await DataGridView.guard(this.result,()=>this.saveRows()))exportDialog(this);}
  async page(direction,limit=pagePreference(this.result)){pageSize(String(limit),this.result.grid?.rowCeiling??1000);const cap=this.result.grid?.capabilities??{};if(!this.transform||!cap.page&&(!cap.rowLimit||!['first','refresh'].includes(direction)))throw Error(cap.rowLimitReason||cap.pageReason||'Row limiting is unavailable for this result.');preferenceState(this.result).local.pageSize=limit;await this.transform({action:'page',direction,limit});}
  async navigate(direction){
    const draft=this.state.draft,page=this.result.grid?.page??{},n=draft.length;
    if(this.searchRows||this.state.localSort||draft.placements?.size){
      let rows=this.displayedRows(),at=rows.indexOf(draft.active);if(!rows.length)return;
      const pageable=!this.searchRows&&this.result.grid?.capabilities.page;
      if(direction==='first'||direction==='last'){
        if(pageable&&(direction==='first'?page.offset>0:page.hasMore)){await this.page(direction);rows=this.displayedRows();}
        const next=direction==='first'?rows[0]:rows.at(-1);this.revealRow(next);return;
      }
      let next=rows[at+(direction==='previous'?-1:1)];
      if(at>=0&&next===undefined&&pageable&&(direction==='previous'?page.offset>0:page.hasMore)){await this.page(direction);rows=this.displayedRows();next=direction==='previous'?rows.at(-1):rows[0];}
      if(next!==undefined){this.selectRow(next);this.revealRow(next);}return;
    }
    if(direction==='first'){if((page.offset??0)>0)await this.page('first');this.scroll.scrollTop=0;this.windowLoader?.reset();this.paintRows?.();return;}
  ...
  adjacentRow(index,delta){const rows=this.displayedRows();return rows[rows.indexOf(index)+delta];}
  revealRow(index){const position=this.displayedRows().indexOf(index);if(position<0)return;const top=position*this.rowHeight,available=this.scroll.clientHeight-(this.find?.state.open?this.find.panel.offsetHeight+12:0);if(top<this.scroll.scrollTop)this.scroll.scrollTop=top;else if(top+2*this.rowHeight>this.scroll.scrollTop+available)this.scroll.scrollTop=Math.max(0,top+2*this.rowHeight-available);this.windowLoader?.reset();this.paintRows?.();}
  cellText(index,column){return formatCell(this.state.draft.cell(index,column),column,this.preferences);}
  rowControls(){
    ...
    if(this.searchRows||this.state.localSort||draft.placements?.size){const rows=this.displayedRows(),at=rows.indexOf(draft.active);const pageable=!this.searchRows&&cap.page;this.navigation.first.disabled=!rows.length||this.scroll.scrollTop<=1&&!(pageable&&page.offset>0);this.navigation.last.disabled=!rows.length||this.scroll.scrollHeight-this.scroll.scrollTop-this.scroll.clientHeight<=1&&!(pageable&&page.hasMore);this.navigation.previous.disabled=at<0||at===0&&!(pageable&&page.offset>0);this.navigation.next.disabled=at<0||at>=rows.length-1&&!(pageable&&page.hasMore);}
    ...
    for(const button of [this.addButton,this.deleteButton])if(button.disabled)button.title=cap.reason||button.getAttribute('aria-label')+' is unavailable for this result';else button.title=button.getAttribute('aria-label');
    if(!this.draftActions){this.draftSave.disabled=!cap.edit||!draft.dirty||!!this.controller?.busy||!!this.result.grid?.uncertain;this.draftRevert.disabled=!draft.dirty||!!this.controller?.busy;}
    const canLimit=!!this.transform&&(cap.rowLimit??cap.page)&&!this.result.grid?.uncertain;this.pageInput.disabled=!canLimit;this.pageInput.title=canLimit?'Rows per page (maximum '+this.result.grid.rowCeiling+'). Press Enter to apply.'+(cap.page?'':' Reloads the first rows only; server paging is unavailable.'):cap.rowLimitReason||this.result.gridUnavailable||'Row limiting is unavailable for this result';this.exportButton.disabled=!n||!this.transform||!!this.result.grid?.refreshRequired;this.rowStatus.title=cap.reason||this.result.gridUnavailable||'';
    if(this.countButton){this.countButton.disabled=!this.result.grid?.capabilities.count||!!this.controller?.busy||!!this.result.grid?.uncertain;this.countButton.title=this.result.grid?.capabilities.count?'Calculate exact filtered query row count':this.result.grid?.capabilities.countReason||'Exact count unavailable';}
    this.rowStatus.textContent=(n?(page.offset??0)+1:0)+'–'+((page.offset??0)+(this.result.rows?.length??0))+(page.total!==undefined?' of '+page.total:'')+' · '+draft.selection.size+' selected'+(draft.dirty?' · '+(draft.changes.size+draft.added.length)+' pending':'')+(this.result.grid?.refreshRequired?' · Saved—refresh required':'')+(this.state.localSort?' · Sorted '+n+' loaded rows':'')+(page.limit!==pagePreference(this.result)?' · '+pagePreference(this.result)+' rows on next fetch':'');if(page.totalCapturedAt)this.rowStatus.title+=' · Count as of '+new Date(page.totalCapturedAt).toLocaleTimeString();
  }
  ...
  editCell(cell,index,column){
    if(this.controller?.busy||!this.state.draft.canEdit(index,column.id)||this.draftActions)return;this.finishCell?.();this.controller?.stopRefresh();
    const draft=this.state.draft,original={...draft.cell(index,column)},dateType=this.preferences.dateEditor?dateInputType(column,original.value):null,input=document.createElement(dateType?'input':'textarea'),kind=document.createElement('select');
    if(dateType){input.type=dateType;input.step='0.001';}input.rows=1;input.wrap='off';input.spellcheck=false;input.maxLength=8192;input.value=dateType==='datetime-local'?String(original.value??'').replace(' ','T'):original.value??'';if(dateType&&original.value&&!input.value){input.type='text';input.value=original.value;}input.setAttribute('aria-label','Edit '+column.label);
    ...
    const changed=()=>{try{draft.set(index,column,{kind:kind.value,...(kind.value==='value'?{value:dateType==='datetime-local'&&String(original.value??'').includes(' ')?input.value.replace('T',' '):input.value}:{})});input.disabled=kind.value!=='value';this.rowControls();input.setCustomValidity('');}catch(error){input.setCustomValidity(error.message);input.reportValidity();}};
    input.oninput=kind.onchange=changed;cell.classList.add('grid-cell-editor');cell.replaceChildren(input);
    // The mode control is portalled outside the cell; its textarea keeps the whole width.
    let ended=false;
    const finish=(render=true)=>{if(ended)return;ended=true;this.cellMode?.destroy();this.cellMode=null;this.finishCell=null;if(render&&!this.disposed)this.renderTable();};this.finishCell=finish;
    if(draft.column(column.id).nullable||draft.column(column.id).hasDefault)this.cellMode=new FloatingCellMode(this,cell,kind,()=>finish());
    cell.onkeydown=kind.onkeydown=event=>{if(event.key==='Escape'){event.preventDefault();draft.set(index,column,original);finish();}else if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();changed();finish();}else if(event.key==='Tab'){event.preventDefault();changed();const columns=this.columns().filter(c=>draft.canEdit(index,c.id)),at=columns.findIndex(c=>c.id===column.id)+(event.shiftKey?-1:1);finish();let nextColumn=columns[at],nextRow=index;if(!nextColumn){nextRow=this.adjacentRow(nextRow,event.shiftKey?-1:1);while(nextRow>=0&&nextRow<draft.length&&draft.deleted(nextRow))nextRow=this.adjacentRow(nextRow,event.shiftKey?-1:1);if(nextRow>=0&&nextRow<draft.length){const candidates=this.columns().filter(c=>draft.canEdit(nextRow,c.id));nextColumn=event.shiftKey?candidates.at(-1):candidates[0];}}if(nextColumn){draft.select(nextRow);draft.activeColumn=nextColumn.id;this.editActiveCell();}}};const blur=()=>queueMicrotask(()=>{const active=document.activeElement;if(!ended&&!cell.contains(active)&&!this.cellMode?.contains(active)&&active!==this.scroll&&!(this.scroll.contains(active)&&active?.classList.contains('column-resize')))finish();});cell.addEventListener('focusout',blur);kind.addEventListener('focusout',blur);input.disabled=kind.value!=='value';(input.disabled?kind:input).focus({preventScroll:true});
  }
  updateDraftActions(){if(!this.draftActions||!this.draftSave)return;const state=this.draftActions.state();this.draftSave.disabled=!state.dirty||state.busy||state.canSave===false;this.draftRevert.disabled=!state.dirty||state.busy;}
  refreshToolbar(){
    const toolbar=document.createElement('div');this.footer=toolbar;toolbar.className='data-grid-footer';toolbar.setAttribute('role','toolbar');toolbar.setAttribute('aria-label','Result refresh');
    ...
    const invoke=action=>Promise.resolve().then(action).catch(error=>{if(this.result.grid?.uncertain)this.renderQueryState();else{this.showError(error.message);}});const save=button('Save','save',()=>invoke(()=>this.draftActions?this.draftActions.save():this.saveRows())),cancel=button(this.draftActions?'Revert':'Cancel',this.draftActions?'undo-2':'x',()=>invoke(()=>{if(this.draftActions)return this.draftActions.revert();this.state.draft.cancel();this.renderTable();}));this.draftSave=save;this.draftRevert=cancel;
    ...
    split.append(status,primary,menuButton);toolbar.append(split);if(this.draftActions||this.rowActions)toolbar.append(divider(),save,cancel);if(this.rowActions)toolbar.append(divider(),add,remove,divider());this.updateDraftActions();
    ...
    const settings=button('Grid settings','settings',()=>this.openGridSettings());this.settingsButton=settings;const countButton=button('Calculate count','hash',()=>invoke(()=>this.transform({action:'count'})));this.countButton=countButton;
    const limit=document.createElement('input');limit.className='grid-page-size';limit.type='text';limit.inputMode='numeric';limit.pattern='[0-9]*';limit.maxLength=7;limit.spellcheck=false;limit.value=String(this.result.grid?.page?.limit??Math.min(200,this.result.grid?.rowCeiling??1000));this.appliedPageSize=this.result.grid?.page?.limit;limit.setAttribute('aria-label','Rows per page');limit.title='Rows per page (maximum '+(this.result.grid?.rowCeiling??1000)+')';limit.oninput=()=>{limit.value=limit.value.replace(/[^0-9]/g,'').slice(0,7);limit.setCustomValidity('');};limit.onkeydown=event=>{if(event.key!=='Enter')return;event.preventDefault();try{const size=pageSize(limit.value,this.result.grid?.rowCeiling??1000);void invoke(()=>this.page('first',size));}catch(error){limit.setCustomValidity(error.message);limit.reportValidity();}};this.pageInput=limit;
    ...
    toolbar.append(navigation,divider(),exportButton,divider(),settings,limit,countButton,this.rowStatus);return toolbar;
  }
  toggleRefreshMenu(){
  ...
  constructor(host,result,{sourceSql=null,notice=()=>{},transform=null,refresh=null,controller=null,structured=null,rowActions=true,draftActions=null,displayOnly=false}={}){this.displayOnly=displayOnly;this.rowActions=rowActions;this.draftActions=draftActions;this.structured=structured;this.host=host;this.result=result;this.sourceSql=sourceSql;this.notice=notice;this.transform=transform;this.refresh=refresh;this.controller=controller;this.state=stateFor(result);this.state.nullText=this.preferences.nullText;this.state.dates=this.preferences.dates;if(controller)controller.cancellationTimeoutMs=this.preferences.cancelTimeout;this.outside=e=>this.handleOutside(e);this.key=e=>this.handleKey(e);this.viewportChanged=()=>{this.closeColumnMenu();this.closeRefreshMenu();};this.state.view=this;this.render();this.state.windowScroll=false;this.windowLoader=new GridScrollWindow(this);this.scroll.addEventListener('copy',event=>{
      if(event.target.closest('input,textarea'))return;
      const selection=getSelection(),node=selection?.anchorNode,cell=(node?.nodeType===1?node:node?.parentElement)?.closest('[role=gridcell]');
      if(!cell||!this.scroll.contains(cell)||!event.clipboardData)return;
      const row=Number(cell.closest('[data-row-index]').dataset.rowIndex),column=this.state.columns.find(c=>c.id===cell.dataset.columnId),value=this.state.draft.cell(row,column);
      event.clipboardData.setData('text/plain',value.kind==='null'?'NULL':value.kind==='default'?'DEFAULT':String(value.value??''));event.preventDefault();
    });this.scroll.addEventListener("click",event=>{if(this.controller?.busy){event.preventDefault();event.stopImmediatePropagation();}},true);document.addEventListener('pointerdown',this.outside,true);document.addEventListener('keydown',this.key,true);window.addEventListener('resize',this.viewportChanged);if(controller)this.unsubscribeQuery=controller.subscribe(()=>this.renderQueryState());}
  destroy(){this.disposed=true;this.settingsDialog?.close();this.valuePicker?.dispose();this.finishCell?.(false);this.state.scroll={left:this.scroll.scrollLeft,top:this.scroll.scrollTop};if(this.state.view===this)delete this.state.view;this.windowLoader?.dispose();this.unsubscribeRefresh?.();this.unsubscribeQuery?.();this.find?.destroy();this.closeRefreshMenu();this.disposed=true;this.host.removeAttribute('aria-busy');document.removeEventListener('pointerdown',this.outside,true);document.removeEventListener('keydown',this.key,true);window.removeEventListener('resize',this.viewportChanged);this.closeColumnMenu();this.closeViewer();this.closeMenu();if(this.dialog?.open)this.dialog.close();this.dialog?.remove();this.valueDialog?.remove();this.host.replaceChildren();}
  render(){this.host.replaceChildren();this.host.className='data-grid-host';const view=document.createElement('section');view.className='data-grid-view';this.command=document.createElement('div');this.command.className='data-grid-command';this.command.append(this.sourceSection(),this.filterSection(),this.actionSection());this.scroll=document.createElement('div');this.scroll.className='data-grid-scroll';this.scroll.tabIndex=0;this.scroll.setAttribute('role','grid');this.scroll.setAttribute('aria-label','Query results');this.live=document.createElement('span');this.live.className='sr-only';this.live.setAttribute('aria-live','polite');this.interactive=document.createElement('div');this.interactive.className='grid-interactive';const footer=this.refreshToolbar();if(this.displayOnly){this.command.hidden=true;for(const child of footer.children)child.hidden=child!==this.settingsButton;}this.stage=document.createElement('div');this.stage.className='grid-data-stage';this.stage.append(this.scroll);this.interactive.append(this.command,this.stage,footer);this.queryError=document.createElement('div');this.queryError.className='grid-query-error';this.queryError.setAttribute('role','alert');this.queryError.hidden=true;view.append(this.queryError,this.interactive,this.live);this.view=view;this.host.append(view);this.renderTable();this.find=new GridSearch(this);if(this.state.scroll){this.scroll.scrollLeft=this.state.scroll.left;this.scroll.scrollTop=this.state.scroll.top;}}
...
actionSection(){const actions=document.createElement('div');actions.className='grid-actions';const split=document.createElement('div');split.className='grid-split';const clear=iconButton('Remove all filtering/orderings','M4 15 14-11 4 5-10 11H7zM13 19h8',()=>this.applyQueryAction('clear_all'));const caret=iconButton('Choose filtering or ordering to remove','M7 9l5 5 5-5',e=>{e.stopPropagation();this.toggleMenu();});caret.classList.add('grid-caret');split.append(clear,caret);this.menu=document.createElement('div');this.menu.className='grid-action-menu';this.menu.hidden=true;for(const label of ['Remove all filtering','Remove all orderings']){const item=document.createElement('button');item.type='button';item.textContent=label;item.addEventListener('click',()=>{this.closeMenu();this.applyQueryAction(label==='Remove all filtering'?'clear_filters':'clear_order');});this.menu.append(item);}split.append(this.menu);const funnel=iconButton('Result Set Order/Filter Settings','M3 5h18l-7 8v5l-4 2v-7z',()=>this.openSettings());const find=iconButton('Find and Replace','M17 10a7 7 0 1 1-14 0 7 7 0 0 1 14 0M15 15l6 6',()=>this.find.toggle());this.findButton=find;find.setAttribute('aria-expanded','false');actions.append(split,funnel,find);return actions;}
  ...
  selectedValue(column){if(this.state.selectedRow===null)return undefined;const cell=this.state.draft.cell(this.state.selectedRow,column);return cell.kind==='null'?null:cell.kind==='value'?cell.value:undefined;}
  valueLabel(value){
  ...
  async applyQueryAction(action,column=null,extra={}){
    if(this.busy)return;
    if(action==='order'&&this.preferences.sorting==='loaded'){this.state.localSort={id:column.id,direction:extra.direction};this.controller?.stopRefresh();this.renderTable();return;}
    if(['clear_all','clear_order'].includes(action)){this.state.localSort=null;this.renderTable();if(action==='clear_order'&&this.preferences.sorting==='loaded')return;}
  ...
  toggleColumnMenu(column,button){
    ...
    add(menu,'Find and Replace','M17 10a7 7 0 1 1-14 0 7 7 0 0 1 14 0M15 15l6 6',false,()=>this.find.open());
    add(menu,'Filter by value',filterIcon,false,()=>{this.valuePicker?.dispose();this.controller?.stopRefresh();this.valuePicker=new GridValuePicker(this,column);});divider();
  ...
  selectRow(index,event={}){this.state.draft.select(index,event);if(event.shiftKey&&(this.searchRows||this.state.localSort||this.state.draft.placements?.size)){const rows=this.displayedRows(),a=rows.indexOf(this.state.draft.anchor),b=rows.indexOf(index);this.state.draft.selection=new Set(rows.slice(Math.min(a,b),Math.max(a,b)+1));const visible=new Set(rows);this.state.draft.selection=new Set([...this.state.draft.selection].filter(row=>visible.has(row)));}this.state.selectedRow=index;this.scroll.querySelectorAll('.grid-body .grid-row').forEach(row=>row.setAttribute('aria-selected',String(this.state.draft.selection.has(Number(row.dataset.rowIndex)))));this.live.textContent='Selected row '+(index+1);this.rowControls();if(this.preferences.highlightMatches)this.updateMatchingValues();this.find?.refreshIfChanged();}
  ...
  get rowNumberWidth(){return this.state.rowNumberWidth??52;}
  fitRowNumbers(){if(this.state.rowNumberResized)return;const digits=String((this.result.grid?.page?.offset??0)+(this.result.rows?.length??0)).length;if(this.state.rowNumberDigits===digits)return;this.state.rowNumberDigits=digits;const measure=document.createElement('canvas').getContext('2d');measure.font=getComputedStyle(this.scroll).font;this.state.rowNumberWidth=Math.max(52,Math.ceil(measure.measureText('8'.repeat(digits)).width)+20);}
  template(){return this.rowNumberWidth+'px '+this.columns().map(column=>(this.state.widths.get(column.id)??180)+'px').join(' ');}
  // One inherited layout for headers and every virtual row, including rows created after a drag.
  applyColumnLayout(){this.scroll.style.setProperty('--grid-column-layout',this.template());}
  ...
  renderTable(focusId=null){
    this.finishCell?.(false);const settings=this.preferences;this.scroll.style.setProperty('--grid-row-height',this.rowHeight+'px');this.scroll.style.setProperty('--grid-font-size',settings.fontSize+'px');this.scroll.dataset.stripes=String(settings.stripes);this.scroll.dataset.lines=settings.gridLines;this.fitRowNumbers();
    if(this.searchRows)this.searchRows=this.searchRows.filter(index=>index<this.state.draft.length);
    ...
    this.applyColumnLayout();const columns=this.columns(),header=document.createElement('div');header.className='grid-row header';header.style.gridTemplateColumns='var(--grid-column-layout)';header.setAttribute('role','row');
    const corner=document.createElement('span');corner.className='row-number row-number-corner';corner.setAttribute('role','columnheader');corner.setAttribute('aria-label','Row numbers');corner.title='Row numbers';const handle=document.createElement('span');handle.className='column-resize';handle.tabIndex=0;handle.title='Resize row numbers column';handle.setAttribute('role','separator');handle.setAttribute('aria-label',handle.title);handle.setAttribute('aria-orientation','vertical');handle.setAttribute('aria-valuemin','36');handle.setAttribute('aria-valuemax','320');handle.setAttribute('aria-valuenow',this.rowNumberWidth);handle.onpointerdown=e=>this.beginColumnResize(e,handle,ROW_NUMBERS);handle.onkeydown=e=>{if(['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();this.resizeColumn(ROW_NUMBERS,this.rowNumberWidth+(e.key==='ArrowLeft'?-10:10),handle);}};handle.ondblclick=()=>{this.state.rowNumberResized=false;this.state.rowNumberDigits=null;this.fitRowNumbers();this.resizeColumn(ROW_NUMBERS,this.rowNumberWidth,handle);this.state.rowNumberResized=false;};corner.append(handle);header.append(corner);
    ...
    const paint=()=>{if(this.finishCell)return;const visible=this.displayedRows(),count=visible.length,start=Math.min(count,Math.max(0,Math.floor((this.scroll.scrollTop-body.offsetTop)/this.rowHeight)-3)),end=Math.min(count,start+Math.ceil(this.scroll.clientHeight/this.rowHeight)+6),topSpace=document.createElement('div'),bottomSpace=document.createElement('div');topSpace.style.height=start*this.rowHeight+'px';bottomSpace.style.height=(count-end)*this.rowHeight+'px';const rows=[];for(let i=start;i<end;i++){const index=visible?visible[i]:i;rows.push(this.makeRow(this.result.rows?.[index]??[],index+1,columns));}body.replaceChildren(topSpace,...rows,bottomSpace);this.rowControls();};this.paintRows=paint;
    this.scroll.onscroll=()=>{this.closeColumnMenu();const editor=this.scroll.querySelector('.grid-cell-editor');if(editor){const bounds=editor.getBoundingClientRect(),viewport=this.scroll.getBoundingClientRect();if(bounds.bottom>viewport.top+this.rowHeight&&bounds.top<viewport.top+this.scroll.clientHeight&&bounds.right>viewport.left+this.rowNumberWidth&&bounds.left<viewport.left+this.scroll.clientWidth){this.cellMode?.position();this.rowControls();return;}this.finishCell?.();}paint();this.windowLoader?.scrolled();};paint();this.scroll.scrollLeft=left;this.scroll.scrollTop=top;this.windowLoader?.reset();paint();this.find?.refreshIfChanged();
  ...
  columnHeader(column){
    ...
    const label=document.createElement('span');label.className='grid-column-label';label.textContent=this.preferences.headings==='labels'||!column.sourceName?column.label:this.preferences.headings==='names'?column.sourceName:column.label===column.sourceName?column.label:column.label+' ('+column.sourceName+')';if(this.preferences.headings!=='labels'&&!column.sourceName)cell.title+=' · Source name unavailable; label retained.';cell.append(label);
  ...
  makeRow(values,rowNumber,columns){const index=rowNumber-1,draft=this.state.draft,row=document.createElement('div');row.className='grid-row';row.classList.toggle('grid-row-deleted',draft.deleted(index));row.classList.toggle('grid-row-added',index>=(this.result.rows?.length??0));row.style.gridTemplateColumns='var(--grid-column-layout)';row.setAttribute('role','row');row.dataset.rowIndex=String(index);row.tabIndex=0;row.setAttribute('aria-selected',String(draft.selection.has(index)));if(draft.deleted(index))row.setAttribute('aria-description','Pending deletion, not yet saved');else if(index>=(this.result.rows?.length??0))row.setAttribute('aria-description','New unsaved row');else if(draft.changes.has(draft.id(index)))row.setAttribute('aria-description','Contains unsaved cell changes');row.addEventListener('click',event=>{if(!event.target.closest('textarea,input,select'))this.selectRow(index,event);});row.addEventListener('keydown',event=>{if(event.target.matches('textarea,input,select'))return;if(['Enter',' '].includes(event.key)){event.preventDefault();this.selectRow(index,event);}else if(event.key==='F2'){event.preventDefault();this.selectRow(index);this.editActiveCell();}else if(['ArrowUp','ArrowDown'].includes(event.key)){event.preventDefault();const delta=event.key==='ArrowUp'?-1:1,next=this.adjacentRow(index,delta);if(next>=0&&next<draft.length){if(next===undefined)return;this.selectRow(next,event);this.revealRow(next);this.scroll.querySelector('[data-row-index="'+next+'"]')?.focus({preventScroll:true});}}});const number=document.createElement('span');number.className='row-number';number.textContent=index>=(this.result.rows?.length??0)?'+':String((this.result.grid?.page?.offset??0)+rowNumber);number.title='Select row '+number.textContent;number.setAttribute('role','rowheader');row.append(number);for(const column of columns){const cell=document.createElement('span'),value=draft.cell(index,column);cell.textContent=this.cellText(index,column);cell.title=cell.textContent;this.find?.highlight(cell,index,column,cell.textContent);const activeColumn=this.state.columns.find(c=>c.id===draft.activeColumn),active=activeColumn&&draft.active!==null?draft.cell(draft.active,activeColumn):null;cell.classList.toggle('grid-value-match',!!(this.preferences.highlightMatches&&active&&activeColumn.jdbcType===column.jdbcType&&active.kind===value.kind&&valueKey(active.value??null,column.jdbcType)===valueKey(value.value??null,column.jdbcType)));cell.setAttribute('role','gridcell');cell.dataset.columnId=column.id;cell.setAttribute('aria-readonly',String(!draft.canEdit(index,column.id)));cell.classList.toggle('grid-cell-changed',!!draft.changes.get(draft.id(index))?.values?.[column.id]);cell.onclick=()=>{draft.activeColumn=column.id;if(this.preferences.highlightMatches)this.updateMatchingValues();};cell.ondblclick=()=>this.editCell(cell,index,column);row.append(cell);}return row;}
  clearDrop(){this.scroll.querySelectorAll('.drop-before,.drop-after').forEach(cell=>cell.classList.remove('drop-before','drop-after'));}
  beginColumnResize(event,handle,id){event.preventDefault();event.stopPropagation();const start=event.clientX,width=id===ROW_NUMBERS?this.rowNumberWidth:this.state.widths.get(id)??180;handle.setPointerCapture(event.pointerId);const move=e=>this.resizeColumn(id,width+e.clientX-start,handle);const up=e=>{handle.removeEventListener('pointermove',move);for(const type of ['pointerup','pointercancel','lostpointercapture'])handle.removeEventListener(type,up);if(handle.hasPointerCapture(e.pointerId))handle.releasePointerCapture(e.pointerId);};handle.addEventListener('pointermove',move);for(const type of ['pointerup','pointercancel','lostpointercapture'])handle.addEventListener(type,up);}
  resizeColumn(id,width,handle){const numbers=id===ROW_NUMBERS;width=Math.max(numbers?36:60,Math.min(numbers?320:800,Math.round(width)));if(numbers){this.state.rowNumberWidth=width;this.state.rowNumberResized=true;}else this.state.widths.set(id,width);this.applyColumnLayout();if(handle)handle.setAttribute('aria-valuenow',width);this.cellMode?.position();}
