...
export class DataGridView {
  ...
  refreshToolbar(){
    ...
    const invoke=action=>Promise.resolve().then(action).catch(error=>this.notice(error.message));const save=button('Save','save',()=>this.draftActions&&invoke(this.draftActions.save)),cancel=button(this.draftActions?'Revert':'Cancel',this.draftActions?'undo-2':'x',()=>this.draftActions&&invoke(this.draftActions.revert));this.draftSave=save;this.draftRevert=cancel;
