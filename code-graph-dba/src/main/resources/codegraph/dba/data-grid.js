...
import {GridValuePicker} from './grid-values.js';
...
export class DataGridView {
  ...
  async page(direction,limit=this.result.grid?.page?.limit??200){pageSize(String(limit),this.result.grid?.rowCeiling??1000);const cap=this.result.grid?.capabilities??{};if(!this.transform||!cap.page&&(!cap.rowLimit||!['first','refresh'].includes(direction)))throw Error(cap.rowLimitReason||cap.pageReason||'Row limiting is unavailable for this result.');await this.transform({action:'page',direction,limit});}
  async navigate(direction){
    const draft=this.state.draft,page=this.result.grid?.page??{},n=draft.length;
    if(this.searchRows){
      const rows=this.searchRows,at=rows.indexOf(draft.active);if(!rows.length)return;
      if(direction==='first'||direction==='last'){this.revealRow(direction==='first'?rows[0]:rows.at(-1));return;}
      const next=rows[at+(direction==='previous'?-1:1)];if(at>=0&&next!==undefined){this.selectRow(next);this.revealRow(next);}return;
    }
  ...
  adjacentRow(index,delta){return this.searchRows?this.searchRows[this.searchRows.indexOf(index)+delta]:index+delta;}
  revealRow(index){const position=this.searchRows?this.searchRows.indexOf(index):index;if(position<0)return;const top=position*28,available=this.scroll.clientHeight-(this.find?.state.open?this.find.panel.offsetHeight+12:0);if(top<this.scroll.scrollTop)this.scroll.scrollTop=top;else if(top+56>this.scroll.scrollTop+available)this.scroll.scrollTop=Math.max(0,top+56-available);this.windowLoader?.reset();this.paintRows?.();}
  cellText(index,column){const value=this.state.draft.cell(index,column);let text=value.kind==='null'?this.state.nullText:value.kind==='default'?'DEFAULT':String(value.value??'');if(this.state.dates==='iso'&&[91,92,93,2013,2014].includes(column.jdbcType))text=text.replace(/^(\d{4}-\d{2}-\d{2}) /,'$1T');return text;}
  rowControls(){
    ...
    if(this.searchRows){const rows=this.searchRows,at=rows.indexOf(draft.active);this.navigation.first.disabled=!rows.length||this.scroll.scrollTop<=1;this.navigation.last.disabled=!rows.length||this.scroll.scrollHeight-this.scroll.scrollTop-this.scroll.clientHeight<=1;this.navigation.previous.disabled=at<=0;this.navigation.next.disabled=at<0||at>=rows.length-1;}
    ...
    const canLimit=!!this.transform&&(cap.rowLimit??cap.page)&&!this.result.grid?.uncertain;this.pageInput.disabled=!canLimit;this.pageInput.title=canLimit?'Rows per page (maximum '+this.result.grid.rowCeiling+'). Press Enter to apply.'+(cap.page?'':' Reloads the first rows only; server paging is unavailable.'):cap.rowLimitReason||this.result.gridUnavailable||'Row limiting is unavailable for this result';this.exportButton.disabled=!n||!this.transform;this.rowStatus.title=cap.reason||this.result.gridUnavailable||'';
  ...
  destroy(){this.disposed=true;this.valuePicker?.dispose();this.finishCell?.(false);this.state.scroll={left:this.scroll.scrollLeft,top:this.scroll.scrollTop};if(this.state.view===this)delete this.state.view;this.windowLoader?.dispose();this.unsubscribeRefresh?.();this.unsubscribeQuery?.();this.find?.destroy();this.closeRefreshMenu();this.disposed=true;this.host.removeAttribute('aria-busy');document.removeEventListener('pointerdown',this.outside,true);document.removeEventListener('keydown',this.key,true);window.removeEventListener('resize',this.viewportChanged);this.closeColumnMenu();this.closeViewer();this.closeMenu();if(this.dialog?.open)this.dialog.close();this.dialog?.remove();this.valueDialog?.remove();this.host.replaceChildren();}
  render(){this.host.replaceChildren();this.host.className='data-grid-host';const view=document.createElement('section');view.className='data-grid-view';this.command=document.createElement('div');this.command.className='data-grid-command';this.command.append(this.sourceSection(),this.filterSection(),this.actionSection());this.scroll=document.createElement('div');this.scroll.className='data-grid-scroll';this.scroll.tabIndex=0;this.scroll.setAttribute('role','grid');this.scroll.setAttribute('aria-label','Query results');this.live=document.createElement('span');this.live.className='sr-only';this.live.setAttribute('aria-live','polite');this.interactive=document.createElement('div');this.interactive.className='grid-interactive';const footer=this.refreshToolbar();if(this.displayOnly){this.command.hidden=true;footer.hidden=true;}this.stage=document.createElement('div');this.stage.className='grid-data-stage';this.stage.append(this.scroll);this.interactive.append(this.command,this.stage,footer);this.queryError=document.createElement('div');this.queryError.className='grid-query-error';this.queryError.setAttribute('role','alert');this.queryError.hidden=true;view.append(this.queryError,this.interactive,this.live);this.view=view;this.host.append(view);this.renderTable();this.find=new GridSearch(this);if(this.state.scroll){this.scroll.scrollLeft=this.state.scroll.left;this.scroll.scrollTop=this.state.scroll.top;}}
...
actionSection(){const actions=document.createElement('div');actions.className='grid-actions';const split=document.createElement('div');split.className='grid-split';const clear=iconButton('Remove all filtering/orderings','M4 15 14-11 4 5-10 11H7zM13 19h8',()=>this.applyQueryAction('clear_all'));const caret=iconButton('Choose filtering or ordering to remove','M7 9l5 5 5-5',e=>{e.stopPropagation();this.toggleMenu();});caret.classList.add('grid-caret');split.append(clear,caret);this.menu=document.createElement('div');this.menu.className='grid-action-menu';this.menu.hidden=true;for(const label of ['Remove all filtering','Remove all orderings']){const item=document.createElement('button');item.type='button';item.textContent=label;item.addEventListener('click',()=>{this.closeMenu();this.applyQueryAction(label==='Remove all filtering'?'clear_filters':'clear_order');});this.menu.append(item);}split.append(this.menu);const funnel=iconButton('Result Set Order/Filter Settings','M3 5h18l-7 8v5l-4 2v-7z',()=>this.openSettings());const find=iconButton('Find and Replace','M17 10a7 7 0 1 1-14 0 7 7 0 0 1 14 0M15 15l6 6',()=>this.find.toggle());this.findButton=find;find.setAttribute('aria-expanded','false');actions.append(split,funnel,find);return actions;}
  ...
  selectRow(index,event={}){this.state.draft.select(index,event);if(event.shiftKey&&this.searchRows){const visible=new Set(this.searchRows);this.state.draft.selection=new Set([...this.state.draft.selection].filter(row=>visible.has(row)));}this.state.selectedRow=index;this.scroll.querySelectorAll('.grid-body .grid-row').forEach(row=>row.setAttribute('aria-selected',String(this.state.draft.selection.has(Number(row.dataset.rowIndex)))));this.live.textContent='Selected row '+(index+1);this.rowControls();this.find?.refreshIfChanged();}
  ...
  renderTable(focusId=null){
    this.finishCell?.(false);this.fitRowNumbers();
    ...
    const paint=()=>{if(this.finishCell)return;const visible=this.searchRows,count=visible?visible.length:this.state.draft.length,start=Math.min(count,Math.max(0,Math.floor((this.scroll.scrollTop-body.offsetTop)/28)-3)),end=Math.min(count,start+Math.ceil(this.scroll.clientHeight/28)+6),topSpace=document.createElement('div'),bottomSpace=document.createElement('div');topSpace.style.height=start*28+'px';bottomSpace.style.height=(count-end)*28+'px';const rows=[];for(let i=start;i<end;i++){const index=visible?visible[i]:i;rows.push(this.makeRow(this.result.rows?.[index]??[],index+1,columns));}body.replaceChildren(topSpace,...rows,bottomSpace);this.rowControls();};this.paintRows=paint;
    this.scroll.onscroll=()=>{this.closeColumnMenu();const editor=this.scroll.querySelector('.grid-cell-editor');if(editor){const bounds=editor.getBoundingClientRect(),viewport=this.scroll.getBoundingClientRect();if(bounds.bottom>viewport.top+28&&bounds.top<viewport.top+this.scroll.clientHeight&&bounds.right>viewport.left+this.rowNumberWidth&&bounds.left<viewport.left+this.scroll.clientWidth){this.cellMode?.position();this.rowControls();return;}this.finishCell?.();}paint();this.windowLoader?.scrolled();};paint();this.scroll.scrollLeft=left;this.scroll.scrollTop=top;this.windowLoader?.reset();paint();this.find?.refreshIfChanged();
