...
export function label(e,m,seen=new Set()){
 ...
 switch(e.kind){case 'column':return (m.sources.find(s=>s.id===e.source)?.alias??'[missing source]')+'.'+e.name;case 'output':{const o=findOutput(m,e.output);if(!o)return '[missing output]';if(seen.has(o.id))return '[circular output]';seen.add(o.id);return o.alias||label(o.expression,m,seen);}case 'literal':return e.value===null?'NULL':e.type==='text'?JSON.stringify(e.value):String(e.value);case 'parameter':return ':'+(m.parameters.find(p=>p.id===e.parameter)?.name??'[missing parameter]');case 'star':return '*';case 'function':return (e.schema?e.schema+'.':'')+(e.package?e.package+'.':'')+e.name+'('+(e.distinct?'DISTINCT ':'')+(e.args??[]).map(a=>label(a,m,new Set(seen))).join(', ')+')';case 'binary':return '('+label(e.left,m,new Set(seen))+' '+e.op+' '+label(e.right,m,new Set(seen))+')';case 'logical':return '('+(e.args??[]).map(a=>label(a,m,new Set(seen))).join(' '+e.op+' ')+')';case 'not':return 'NOT '+label(e.arg,m,seen);case 'null':return label(e.arg,m,seen)+(e.not?' IS NOT NULL':' IS NULL');case 'between':return label(e.arg,m,seen)+' BETWEEN '+label(e.lower,m,new Set(seen))+' AND '+label(e.upper,m,new Set(seen));case 'in':return label(e.arg,m,seen)+' IN ('+(e.args??[]).map(a=>label(a,m,new Set(seen))).join(', ')+')';case 'case':return 'CASE '+(e.branches??[]).map(b=>'WHEN '+label(b.when,m,new Set(seen))+' THEN '+label(b.then,m,new Set(seen))).join(' ')+' ELSE '+label(e.else,m,new Set(seen))+' END';default:return 'Choose expression';}
}
