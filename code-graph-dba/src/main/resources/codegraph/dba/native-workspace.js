...
export class NativeWorkspace {
  constructor({api,profile,state,changed,account}) {
    ...
    if(profile.transport==='redis'){
      ...
      this.streamButton=this.button(toolbar,'Compose Redis stream entry','list-plus',()=>this.openValueEditor('stream'));
  ...
  openValueEditor(kind){
    ...
    this.valueEditorBytes=(stream?512:256)*1024;
    ...
    if(stream)try{const command=JSON.parse(this.editor.value);if(Array.isArray(command)&&['XLEN','XRANGE','XREVRANGE','XADD','XDEL','XTRIM'].includes(command[0]?.toUpperCase()))key=command[1]??'';}catch{}
    const Editor=stream?RedisStreamEditor:set||sorted?RedisMemberEditor:RedisStringEditor;
  ...
  showResult(result){
    ...
    try{this.retain(cost);}catch(error){
      ...
      for(const key of ['operation','outcome','target','deliveredIds','deletedPendingIds','deletedPendingIdsReported','nextCursor','scanComplete','acknowledgementRequired','pendingDeliveryStateChanged','value','entryId','applied','existingStreamOnly'])if(Object.hasOwn(result,key))receipt[key]=result[key];
