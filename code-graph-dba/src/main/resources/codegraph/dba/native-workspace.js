...
export class NativeWorkspace {
  constructor({api,profile,state,changed,account}) {
    ...
    if(profile.transport==='redis'){
      ...
      this.listButton=this.button(toolbar,'Edit Redis list item','square-pen',()=>this.openStringEditor(false,true));
  ...
  openStringEditor(hash=false,list=false){
    ...
    this.stringEditor=new RedisStringEditor({key,...(list?{listIndex}:hash?{field}:{}),run:(command,expectedTargetRevision)=>this.run(false,{command,expectedTargetRevision}),readOnly:()=>this.profile.readOnly!==false,changed:()=>this.changed(),close:()=>{this.stringEditor.dispose();this.stringEditor=null;this.lockStringTarget();this.retain(this.displayBytes);this.changed();}});
