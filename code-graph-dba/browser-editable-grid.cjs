...
module.exports=async(browser,base,jar)=>{
  ...
  try{
    ...
    await footer.getByRole('button',{name:'Grid settings',exact:true}).click();const settings=page.getByRole('dialog',{name:'Grid settings'});await settings.getByRole('tab',{name:'Columns',exact:true}).click();await settings.locator('.grid-settings-columns input[type=checkbox]').last().uncheck();await settings.getByRole('button',{name:'Apply and Close',exact:true}).click();assert.equal(await page.locator('.data-column-header').count(),3);
    ...
    await footer.getByRole('button',{name:'Save',exact:true}).click();await page.waitForFunction(()=>document.querySelector('#grid')?.getAttribute('aria-busy')==='false');await footer.getByRole('button',{name:'Calculate count',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.grid-row-status')?.textContent.includes('of 452'));
    ...
    await page.waitForFunction(()=>document.querySelector('#grid')?.getAttribute('aria-busy')==='false');await footer.getByRole('button',{name:'Calculate count',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.grid-row-status')?.textContent.includes('of 451'));
