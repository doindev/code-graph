param([string]$BuildRoot=(Split-Path $PSScriptRoot), [string[]]$Vendors=@('postgresql','mysql','mariadb'))
$ErrorActionPreference='Stop'
$ownedRun='cgraph-reusable-qa-'+[guid]::NewGuid().ToString('N')
$ownedContainers=@()
$introducedImages=@()
$previousEnv=@{}
$envNames=@('DBA_REUSABLE_DISPOSABLE','DBA_REUSABLE_VENDOR','DBA_REUSABLE_URL','DBA_REUSABLE_JAR','DBA_REUSABLE_USER','DBA_REUSABLE_PASSWORD')
...
try {
    foreach($vendor in $Vendors){
        ...
        if($LASTEXITCODE -ne 0){$introducedImages+=$image;docker pull $image;if($LASTEXITCODE -ne 0){throw "Could not pull $image"}}
        $name="$ownedRun-$vendor"
        ...
        $env:DBA_REUSABLE_URL='jdbc:'+$vendor+'://127.0.0.1:'+$hostPort+'/approval_test'
        if($vendor -eq 'mysql'){$env:DBA_REUSABLE_URL+='?allowPublicKeyRetrieval=true&useSSL=false'}
        $jar=switch($vendor){
        ...
        & mvn -f (Join-Path $BuildRoot 'pom.xml') -pl code-graph-dba -am test '-Dtest=ReusableVendorIntegrationTest' '-Dsurefire.failIfNoSpecifiedTests=false' '-Djava.awt.headless=true'
    ...
    foreach($name in $ownedContainers){
        $owner=docker inspect --format '{{index .Config.Labels "codegraph.reusable.owner"}}' $name 2>$null
        if($LASTEXITCODE -eq 0 -and $owner.Trim() -eq $ownedRun){docker rm --force --volumes $name}
    }
    foreach($image in $introducedImages){
        $users=docker ps --all --quiet --filter "ancestor=$image"
        if(!$users){docker image rm $image}
    }
