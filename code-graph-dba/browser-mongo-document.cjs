...
module.exports=async(browser,base)=>{
  ...
  try{
    ...
    await page.evaluate(async()=>{
      ...
      const api=async(path,method,input)=>{
        ...
        if(path==='/jobs/document-job'){
          ...
          let result;
          if(f.command.transaction)result={kind:'transaction',atomic:true,documentGuard:!f.badReceipt,outcome:f.fail?'rollback_acknowledged':'commit_acknowledged',entries:[{index:0,operation:f.command.transaction[0].insert?'insert':f.command.transaction[0].delete?'delete':'update',state:f.fail?'rolled_back':'committed',matchedOrInserted:1,modified:1}]};
    ...
    await page.setViewportSize({width:1280,height:900});await open.click();
    const create=editor.getByRole('button',{name:'New document',exact:true}),beforeNew=await page.evaluate(()=>({requests:mongoEditorFixture.requests.length,applies:mongoEditorFixture.applies}));
    await create.click();await status.filter({hasText:'New document draft'}).waitFor();assert.equal(await page.evaluate(()=>mongoEditorFixture.requests.length),beforeNew.requests+1);assert.equal(await page.evaluate(()=>mongoEditorFixture.applies),beforeNew.applies);assert.equal(await save.isDisabled(),false);assert.equal(await remove.isDisabled(),true);
    const suggested=JSON.parse(await value.inputValue());assert.match(suggested._id.$oid,/^[0-9a-f]{24}$/);assert.ok(Math.abs(parseInt(suggested._id.$oid.slice(0,8),16)-Date.now()/1000)<10);
    const newDocument={_id:'explicit-new-id',large:{$numberLong:'9007199254740993'},price:{$numberDecimal:'1.2300'},nullable:null};await value.fill(JSON.stringify(newDocument));assert.equal(await editor.getByRole('textbox',{name:'Document _id as Extended JSON'}).inputValue(),'"explicit-new-id"');
    await page.evaluate(()=>{const w=mongoEditorFixture.workspace;w.unmount();w.mount(document.querySelector('#mongo-editor-fixture'));});assert.deepEqual(JSON.parse(await value.inputValue()),newDocument);
    page.once('dialog',d=>d.dismiss());await close.click();assert.equal(await editor.isVisible(),true);
    await save.click();await review.waitFor();assert.match(await review.innerText(),/absentId/);await review.getByRole('button',{name:'Cancel',exact:true}).click();await status.filter({hasText:'Cancelled before execution'}).waitFor();assert.equal(await page.evaluate(()=>mongoEditorFixture.applies),beforeNew.applies);
    await page.evaluate(()=>mongoEditorFixture.fail=true);await save.click();await review.waitFor();await review.getByRole('button',{name:'Apply once'}).click();await status.filter({hasText:'Document changed concurrently'}).waitFor();assert.deepEqual(JSON.parse(await value.inputValue()),newDocument);assert.equal(await save.isDisabled(),false);
    await page.evaluate(()=>{mongoEditorFixture.fail=false;mongoEditorFixture.lostReply=true;});await save.click();await review.waitFor();await review.getByRole('button',{name:'Apply once'}).click();await status.filter({hasText:'outcome uncertain'}).waitFor();assert.equal(await create.isDisabled(),true);assert.equal(await save.isDisabled(),true);assert.equal(await revert.isDisabled(),true);
    page.once('dialog',d=>d.accept());await reconcile.click();await page.evaluate(()=>mongoEditorFixture.lostReply=false);await create.click();await status.filter({hasText:'New document draft'}).waitFor();await value.fill(JSON.stringify(newDocument));
    await save.click();await review.waitFor();await review.getByRole('button',{name:'Apply once'}).click();await status.filter({hasText:'Document created; transaction commit confirmed'}).waitFor();
    const inserted=await page.evaluate(()=>mongoEditorFixture.requests.at(-1));assert.deepEqual(inserted.command.transaction,[{insert:'items',documents:[newDocument]}]);assert.deepEqual(inserted.command.documentGuard,{collectionUuid:'AAAAAAAAAAAAAAAAAAAAAA==',absentId:'explicit-new-id'});assert.equal(await page.evaluate(()=>mongoEditorFixture.workspace.dirty),false);assert.equal(await value.getAttribute('readonly'),'');assert.equal(await save.isDisabled(),true);
    await create.click();await status.filter({hasText:'New document draft'}).waitFor();const pristine=await value.inputValue();await value.fill(JSON.stringify(newDocument));await revert.click();assert.equal(await value.inputValue(),pristine);assert.equal(await save.isDisabled(),false);
    await page.evaluate(()=>mongoEditorFixture.workspace.stringEditor.stageDelete());assert.equal(await remove.getAttribute('aria-pressed'),'false');page.once('dialog',d=>d.accept());await close.click();
    await page.evaluate(()=>mongoEditorFixture.view=true);await open.click();await create.click();await status.filter({hasText:'existing ordinary collection'}).waitFor();assert.equal(await save.isDisabled(),true);await page.evaluate(()=>mongoEditorFixture.view=false);await close.click();
    await page.evaluate(()=>mongoEditorFixture.workspace.profile.readOnly=true);await open.click();assert.equal(await create.isDisabled(),true);const beforeReadOnly=await page.evaluate(()=>mongoEditorFixture.requests.length);await page.evaluate(()=>mongoEditorFixture.workspace.stringEditor.load(true));assert.equal(await page.evaluate(()=>mongoEditorFixture.requests.length),beforeReadOnly);await close.click();await page.evaluate(()=>mongoEditorFixture.workspace.profile.readOnly=false);
