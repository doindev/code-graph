...
Guarded saves require a replica-set profile and existing ordinary collection;
...
| Redis | Bounded pipeline object | 1–32 scalar/range commands, up to 100 key references; single-slot Cluster execution; one dispatch, no atomicity/rollback/replay; complete per-command receipts under result limits. See [pipeline API and recovery](redis-pipelines.md) |
| Redis | SETBIT, BITCOUNT/BITPOS, BITFIELD/BITFIELD_RO, PFADD/PFCOUNT/PFMERGE, GEOADD/GEODIST/GEOPOS/GEOHASH/GEOSEARCH/GEOSEARCHSTORE | Bit writes address at most 64 KiB, at most 32 bitfield operations, geo COUNT at most 100, multi-key Cluster commands require one slot. PFCOUNT is a reviewed write. See [bounded grammar and recovery](redis-values.md) |
