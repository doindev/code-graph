...
Run one structurally restricted SELECT asynchronously with at most 100 rows/1 MiB. If operationId is returned, poll dba_request_status with that ID until terminal. A queued operation needs no agent-side intervention; do not resubmit it.
...
Read capped metadata. Use bindingId or connectionId plus exact connectionName. If operationId is returned, poll dba_request_status with that ID until terminal. A queued operation needs no agent-side intervention; do not resubmit it.
...
Inspect supported PostgreSQL, MySQL/MariaDB and SQL Server object definitions through scoped catalog permission; partial or inaccessible definitions are labeled. Never creates objects. Returns an asynchronous operation or job. If operationId is returned, poll dba_request_status with that ID until terminal. A queued operation needs no agent-side intervention; do not resubmit it.
