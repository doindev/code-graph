...
Each project generation has a unique temporary MVStore file inside a session directory created under the OS temporary directory. Project roots overlapping that directory are rejected. No complete second Java graph, project-wide fragment collection or resolver table is retained in hybrid mode.

Indexing stages fragments and name indexes on disk, resolves one fragment at a time, then publishes the complete graph atomically. Reads continue using the previous generation during the build; publication waits for active compound reads. Failed extraction or staging leaves the old generation unchanged and removes the staged file.

**Tradeoff:** filesystem changes currently trigger a bounded full rebuild, not the in-memory backend's incremental delta algorithm. Rebuilds can be substantially slower and temporarily need disk space for two generations. First-time substring searches and whole-graph projections also scan disk; repeated exact search requests are cached. Prefer scoped symbol/caller/impact queries for latency-sensitive use.
