...
final class DbaInputSchemas {
    static void enrich(String operation, ObjectNode schema) {
        ...
        if(operation.equals("dba_request_native_command")){
            ...
            command.put("description",command.path("description").asText()+" Redis also accepts {transaction: [argument arrays], watch: [{key, expected}]}: 1..32 supported mutations, at most 16 string/absent, hash-field or list-position expectations and 100 total key references. expected is text/base64 or null for absence. List expectations require index 0..9999, length 1..10000 greater than index, and non-null complete expected bytes; they cannot combine with field. LSET replaces only an existing position; current length/value checks are not stable identities or change history. One Cluster hash slot; exact review, no rollback or automatic retries.");
            ...
            var ordinary=watchAlternatives.addObject().putObject("not").putArray("anyOf");ordinary.addObject().putArray("required").add("index");ordinary.addObject().putArray("required").add("length");
