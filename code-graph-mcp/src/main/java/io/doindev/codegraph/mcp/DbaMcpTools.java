...
public final class DbaMcpTools {
    ...
    private static List<GraphTool> tools(DbaRuntime runtime,Supplier<String> stdioPrincipal,boolean editor){
        ...
        tools.add(tool(runtime,stdioPrincipal,"dba_list_connections","List stable connection IDs/names without credentials or URLs. Trusted local clients can discover connections without setup; database access is checked separately. Optional named-token clients retain their grant-filtered roster.",List.of(),Map.of()));
        tools.add(tool(runtime,stdioPrincipal,"dba_get_metadata","Read capped metadata. Use bindingId or connectionId plus exact connectionName.",List.of("connectionName"),Map.of("schema","string","object","string")));
        tools.add(tool(runtime,stdioPrincipal,"dba_get_object_ddl","Inspect supported PostgreSQL, MySQL/MariaDB and SQL Server object definitions through scoped catalog permission; partial or inaccessible definitions are labeled. Never creates objects. Returns an asynchronous operation or job.",List.of("connectionName","schema","object"),Map.of()));
        tools.add(tool(runtime,stdioPrincipal,"dba_explain_query","Get an estimated SELECT plan without execution-based analysis under legacy or scoped explain permission. Returns an asynchronous operation or job.",List.of("connectionName","sql"),Map.of("parameters","array")));
        ...
        if(editor){
            tools.add(tool(runtime,stdioPrincipal,"dba_request_editor_access","Connect this MCP session to one DBA browser workspace. Poll dba_request_status with the returned operationId until paired. Requires a live logical MCP session. Never grants SQL execution, file saving or database permissions.",List.of("requestId","purpose"),Map.of()));
        ...
        if(approvals){
            ...
            tools.add(tool(runtime,stdioPrincipal,"dba_request_live_sql","Submit SQL using bindingId, or standalone connectionId plus exact connectionName with optional database/schema. Returns an asynchronous operation. Unknown execution outcomes are never automatically replayed.",List.of("requestId","sql","purpose"),Map.of("bindingId","string","connectionId","string","connectionName","string","database","string","schema","string","parameters","array","autoCommit","boolean")));
            tools.add(tool(runtime,stdioPrincipal,"dba_cancel_live_request","Compatibility alias for dba_cancel_request; use the canonical tool with operationId.",List.of("approvalId"),Map.of()));
            tools.add(tool(runtime,stdioPrincipal,"dba_live_request_status","Compatibility alias for dba_request_status; use the canonical tool with operationId.",List.of("approvalId"),Map.of()));
        }
            tools.add(tool(runtime,stdioPrincipal,"dba_request_status","Poll an asynchronous operation using the server-returned operationId. Queued is nonterminal; continue polling until complete, paired, failed or cancelled. A submitted operation may include a job and jobId. Do not resubmit pending work. Legacy ID aliases remain accepted.",List.of("requestId"),Map.of()));
            tools.add(tool(runtime,stdioPrincipal,"dba_cancel_request","Cancel this agent's queued operation or request cancellation of its running job using operationId. Legacy ID aliases remain accepted.",List.of("requestId"),Map.of()));
    ...
    private static GraphTool tool(DbaRuntime runtime,Supplier<String> stdio,String name,String description,List<String> required,Map<String,String> optional){
        ...
        if (name.startsWith("dba_request_") && !Set.of("dba_request_status","dba_request_editor_access").contains(name) || Set.of("dba_get_metadata","dba_get_object_ddl","dba_explain_query","dba_analyze_query_plan","dba_execute_read_query","dba_get_connection_details").contains(name))
            description+=" If operationId is returned, poll dba_request_status with that ID until terminal. A queued operation needs no agent-side intervention; do not resubmit it.";
    ...
    static final class AgentTool implements GraphTool {
        ...
        ToolResponse execute(String transportPrincipal,String transportSession,JsonNode args){try{String principal=stdio!=null?stdio.get():transportPrincipal;String session=stdio!=null?stdioSession:transportSession;JsonNode result=runtime.agentToolCall(principal,session==null||session.isBlank()?null:session,spec.name(),args);boolean error=Set.of("failed","cancelled").contains(result.path("state").asText());return new ToolResponse(result.toString(),error);}catch(IllegalStateException e){if(e.getMessage()!=null&&e.getMessage().startsWith("approval_unavailable:"))return ToolResponse.fail("{\"error\":{\"code\":\"approval_unavailable\",\"message\":\"No interactive approval channel is available\"}}");return ToolResponse.fail("DBA operation unavailable");}catch(SecurityException e){return ToolResponse.fail("DBA access denied; authenticate and check current access");}catch(IllegalArgumentException e){String message=e.getMessage();return ToolResponse.fail(message!=null&&message.startsWith("Approval queue is full")?"Operation queue is full; wait for existing operations":message);}catch(Exception e){return ToolResponse.fail("DBA operation failed");}}
    }
}
